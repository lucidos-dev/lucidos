...
async function captureAppUIInner(appId: string, requestId: string): Promise<void> {
  ...
  // Wait for cognos._capture to become available (iframe may still be loading)
  type CognosCaptureApi = { _capture: () => Promise<{ screenshot: string; dom: string }> };
  type CognosWindow = Window & { cognos?: Partial<CognosCaptureApi> };
  let cognosApi: CognosCaptureApi | null = null;
  for (let i = 0; i < 15; i++) {
    const candidate = (iframe.contentWindow as CognosWindow | null)?.cognos;
    if (candidate?._capture) {
      cognosApi = candidate as CognosCaptureApi;
      break;
    }
  ...
  const result = await Promise.race([cognosApi._capture(), captureTimeout]);
