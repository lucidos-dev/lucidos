...
describe('normalizeDataPath', () => {
  it('prepends artifacts/ when path has no known prefix', () => {
    expect(normalizeDataPath('research/cognos-product-market-fit.md')).toBe(
      'artifacts/research/cognos-product-market-fit.md',
    );
  });
  ...
  it('leaves system-docs/ paths unchanged', () => {
    expect(normalizeDataPath('system-docs/best-practices.md')).toBe(
      'system-docs/best-practices.md',
    );
  });
