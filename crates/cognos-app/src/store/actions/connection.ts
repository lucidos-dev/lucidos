import { connectionStatus, dismissToast, showToast, isProcessing, workspaceName, workspacePath, engineStartedAt, lucidosRelease, engineVersion, latestEngineVersion, latestTauriAppVersion, updateAvailable, panelOverlay, activeInlineForm, focusedThreadId, threadMap, engineRestarting, threadsLoaded, restartRequired } from '../store';
...
export async function checkConnection(): Promise<boolean> {
  ...
  if (health) {
    ...
    if (health.latest_tauri_app_version) {
      ...
      const currentAppVersion = window.__COGNOS_APP_VERSION__;
