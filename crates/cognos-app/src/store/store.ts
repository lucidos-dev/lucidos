...
export const filePreviewSource = signal(false);
...
// --- Settings subview ---
export type SettingsSubview = 'main' | 'models' | 'appearance' | 'memory' | 'devices' | 'accounts' | 'backup' | 'repositories';
...
// --- Active menu item ---
const _savedMenuItem = localStorage.getItem('cognos-active-menu-item');
...
export const animationSpeed = signal(
  parseInt(localStorage.getItem('cognos-animation-speed-slider') || '0', 10) || 0
);
...
export const threadDrawerOpen = signal(
  localStorage.getItem('cognos-thread-drawer-open') === 'true'
);
export const DEFAULT_DRAWER_WIDTH = 300;
export const MIN_DRAWER_WIDTH = 200;
export const threadDrawerWidth = signal(
  Number(localStorage.getItem('cognos-thread-drawer-width')) || DEFAULT_DRAWER_WIDTH
);
...
function restoreThreadChannelFilter(): Set<ThreadChannel> {
  try {
    const saved = localStorage.getItem('cognos-thread-channel-filter');
    if (saved) {
...
export const splitRatio = signal(
  parseFloat(localStorage.getItem('cognos-split-ratio') || '0.4')
);
// --- Mobile view ---
...
const savedMobileView = localStorage.getItem('cognos-mobile-view') as MobileView | null;
...
export function setMobileView(view: MobileView) {
  ...
  localStorage.setItem('cognos-mobile-view', view);
}
...
function restoreInputMode(): InputMode {
  try {
    const saved = localStorage.getItem('cognos-input-mode');
    if (saved) {
  ...
  const old = localStorage.getItem('cognos-input-target');
...
export const selectedRepoId = signal<string>(localStorage.getItem('cognos-cc-last-repo') ?? '');  // '' = CognOS (default)
...
/** Encode a repo file path for the panel overlay. */
export function encodeRepoPath(repoId: string, mode: 'file' | 'diff', path: string): string {
  return `repo:${repoId}:${mode}:${path}`;
}

/** Decode a repo file path from the panel overlay. Returns null if not a repo path. */
export function parseRepoPath(encoded: string): { repoId: string; mode: 'file' | 'diff'; path: string } | null {
  if (!encoded.startsWith('repo:')) return null;
  const [, repoId, mode, ...rest] = encoded.split(':');
  return { repoId, mode: mode as 'file' | 'diff', path: rest.join(':') };
}
...
export const stepsExpanded = signal(
  localStorage.getItem('cognos-steps-expanded') === 'true'
);
export const detailsExpanded = signal(
  localStorage.getItem('cognos-details-expanded') === 'true'
);
export const contextExpanded = signal(
  localStorage.getItem('cognos-context-expanded') === 'true'
);

/** Set of "threadId:userSeq" keys whose responses are collapsed (hidden). */
export const collapsedExchanges = signal<Set<string>>(
  (() => {
    try {
      const saved = localStorage.getItem('cognos-collapsed-exchanges');
...
export function toggleExchangeCollapsed(threadId: string, userSeq: number): void {
  const key = `${threadId}:${userSeq}`;
  const next = new Set(collapsedExchanges.value);
  if (next.has(key)) {
    next.delete(key);
  } else {
    next.add(key);
  }
  collapsedExchanges.value = next;
}

// --- Upload progress ---
export type UploadProgress =
  | { status: 'uploading'; filename: string; current: number; total: number }
  | { status: 'done'; succeeded: number; failed: number; errors: string[] };
export const uploadProgress = signal<UploadProgress | null>(null);
...
export const expandedFolders = signal<Set<string>>(
  (() => {
    try {
      const saved = localStorage.getItem('cognos-expanded-folders');
      return saved ? new Set(JSON.parse(saved) as string[]) : new Set<string>();
    } catch {
      return new Set<string>();
    }
...
export const unreadCount = signal(parseInt(localStorage.getItem('cognos-unread-count') || '0', 10) || 0);
const cachedFilter = localStorage.getItem('cognos-notifications-filter');
...
export function showConfirm(
  message: string,
  okLabel = 'Delete',
  options?: { extraAction?: ToastAction; variant?: 'danger' | 'default'; details?: ConfirmDetails }
): Promise<boolean> {
  return new Promise((resolve) => {
    confirmState.value = { visible: true, message, okLabel, variant: options?.variant, resolve, extraAction: options?.extraAction, details: options?.details };
  });
}
...
export type MessageRoutePanelSection = 'origin' | 'executor';
