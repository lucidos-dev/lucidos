...
// ---------------------------------------------------------------------------
// Message sender / route label
// ---------------------------------------------------------------------------
describe('exchangeUserSource — reads MessageReceived.sender', () => {
  it('user-typed MessageReceived returns "user"', () => {
    const { map, id } = makeThread();
    insertEvents(map, id, [
      { type: 'MessageReceived', text: 'hi', sender: 'user' } as any,
    ]);
    const exchanges = getExchanges(map, id);
    expect(exchangeUserSource(exchanges[0])).toBe('user');
  });

  it('system-emitted MessageReceived returns "system"', () => {
    const { map, id } = makeThread();
    insertEvents(map, id, [
      { type: 'MessageReceived', text: 'auto', sender: 'system' } as any,
    ]);
    const exchanges = getExchanges(map, id);
    expect(exchangeUserSource(exchanges[0])).toBe('system');
  });

  it('MessageReceived without sender defaults to "user"', () => {
    const { map, id } = makeThread();
  ...
  it('MessageReceived with legacy `source` field still works', () => {
    const { map, id } = makeThread();
    insertEvents(map, id, [
      { type: 'MessageReceived', text: 'hi', source: 'system' } as any,
    ]);
    const exchanges = getExchanges(map, id);
    expect(exchangeUserSource(exchanges[0])).toBe('system');
  });
...
describe('Route label — system-initiated thread, user follow-up', () => {
  it('user follow-up in scheduled-trigger CC thread renders "User → Claude Code"', () => {
    ...
    insertEvents(map, id, [
      ...
      { type: 'ResponseGenerated' },
      { type: 'CodingAgentIdled' },
      { type: 'MessageReceived', text: 'what model is used?', channel: 'claude_code', sender: 'user' } as any,
    ]);
...
describe('Flow: SessionRecovered recovery', () => {
  it('SessionRecovered acts as exchange boundary, thread shows CC content', () => {
    ...
    expect(exchangeUserMessage(exchanges[0])).toBe('Session resumed after engine restart');
