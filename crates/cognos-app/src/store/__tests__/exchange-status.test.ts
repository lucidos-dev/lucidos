...
describe('exchangeStatus — full branch coverage', () => {
  ...
  it('returns "aborted" when SessionEnded without response or idle', () => {
    // SessionEnded alone does NOT complete an exchange — there must be a
    // ResponseGenerated (chat) or CodingAgentIdled (CC) to count as done.
    const exchange = makeExchange(msg(), [
      step(1, { type: 'SessionStarted', session_id: 's1' }),
      step(2, { type: 'SessionEnded' }),
    ]);
  ...
  it('returns "aborted" for chat exchange with SessionEnded but no ResponseGenerated', () => {
    const exchange = makeExchange(msg(), [
      step(1, { type: 'SessionEnded' }),
    ]);
    expect(exchangeStatus(exchange, '', true)).toBe('aborted');
  });
