...
function deviceIdHeader(): Record<string, string> {
  ...
  const id = localStorage.getItem('cognos-device-id');
  return id ? { 'x-cognos-device-id': id } : {};
}

/** `fetch` wrapped to always send `x-cognos-device-id` so the engine can
...
export async function json<T>(url: string, init?: RequestInit, timeoutMs = 10000): Promise<T> {
  ...
  try {
    ...
    if (!res.ok) {
      // Try to extract error message from response body (e.g. { "error": "..." })
...
export async function cancelChat(threadId?: string): Promise<void> {
  ...
  const res = await mutatingFetch(`${API}/chat/cancel${params}`, { method: 'POST' });
...
export async function applyNow(threadId: string): Promise<void> {
  ...
  if (!res.ok) {
    throw new ApiError(res.status, 'Failed to start apply');
  }
}
...
export async function cancelClaudeCode(apply?: boolean, threadId?: string, discard?: boolean): Promise<void> {
  ...
  const res = await mutatingFetch(url, { method: 'POST' });
  if (!res.ok) {
    throw new ApiError(res.status, 'Failed to cancel Claude Code session');
  }
}
 ...
 *  `ApplyResult` struct in `crates/cognos-engine/src/engine/types.rs`. */
...
export async function restartEngine(): Promise<void> {
  await mutatingFetch(`${API}/restart`, { method: 'POST' });
}
...
export function listArtifacts(): Promise<ArtifactsResponse> {
  return cognos.data.list().then(files => ({
    artifacts: files,
  })) as Promise<ArtifactsResponse>;
}

export function uploadFile(file: File): Promise<UploadResponse> {
  return cognos.data.upload(file) as Promise<UploadResponse>;
}
...
export function getNotifications(params?: {
  ...
  return cognos.notifications.list(params) as Promise<NotificationsResponse>;
}
...
export function markNotificationRead(id: string): Promise<void> {
  return cognos.notifications.markRead(id);
}

export function markAllNotificationsRead(): Promise<void> {
  return cognos.notifications.markAllRead();
}
...
export function listAppsApi(): Promise<App[]> {
  return cognos.apps.list() as Promise<App[]>;
}
...
export async function getPreferences(deviceId?: string): Promise<{ preferences: Record<string, string> }> {
  const preferences = await cognos.preferences.get(deviceId);
  return { preferences };
}

export async function setPreference(key: string, value: string, deviceId?: string): Promise<ApiResult> {
  await cognos.preferences.set(key, value, deviceId);
  return { success: true };
}
...
export function listTriggers(): Promise<TriggersListResponse> {
  return cognos.triggers.list().then(triggers => ({ triggers })) as Promise<TriggersListResponse>;
}
...
export function createTrigger(body: {
  ...
  return cognos.triggers.create(body) as Promise<ApiResult>;
}
...
export function updateTrigger(
  ...
  return cognos.triggers.update(id, body) as Promise<ApiResult>;
}

export function deleteTriggerApi(
  id: string
): Promise<ApiResult> {
  return cognos.triggers.delete(id) as Promise<ApiResult>;
}
...
export async function getRepoDiff(repoId: string, branch: string): Promise<RepoDiff> {
  return json(`${API}/repositories/${encodeURIComponent(repoId)}/diff?branch=${encodeURIComponent(branch)}`);
}
