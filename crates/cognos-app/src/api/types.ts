...
/**
 * Who originated a chat message. Required on every ChatRequest — the backend
 * rejects requests without it. `'system'` additionally requires
 * `parent_thread_id` (and ideally `spawning_event_id`) so a system-spawned
 * thread always records what spawned it.
 */
export type ChatSender = 'user' | 'system';

export interface ChatRequestBody {
  message: string;
  /** Required: who originated this message. */
  sender: ChatSender;
  model?: string;
  device_id?: string;
  ...
  event_id?: string;
  thread_id?: string;
  /** Required when `sender === 'system'`: the thread that spawned this one. */
  parent_thread_id?: string;
  /** Required when `sender === 'system'`: the parent event that triggered the spawn. */
