...
function addImageFile(file: File) {
  ...
  reader.onload = () => {
    ...
    addPastedImage(id, { base64, mimeType });
    syncDraftEntry(id);
  };
...
export function PromptInput() {
  ...
  useEffect(() => {
    ...
    if (prev === undefined) {
      const oldDraft = localStorage.getItem('cognos-draft');
      if (oldDraft) {
        saveDraftTextStorage(did, oldDraft);
        localStorage.removeItem('cognos-draft');
        syncDraftEntry(did);
      }
  ...
  // Re-measure height when root font metrics change (UI scale, font family).
  // autoResize() sets height in px — goes stale when root font-size changes
  // (e.g. after loadPreferences applies the user's UI scale on page reload).
  useEffect(() => {
    let lastScale = document.documentElement.style.getPropertyValue('--user-ui-scale');
    let lastFont = document.documentElement.style.getPropertyValue('--font-ui');
    let rafId: number | null = null;

    const observer = new MutationObserver(() => {
      const scale = document.documentElement.style.getPropertyValue('--user-ui-scale');
      const font = document.documentElement.style.getPropertyValue('--font-ui');
      if (scale !== lastScale || font !== lastFont) {
        lastScale = scale;
        lastFont = font;
        if (rafId === null) {
          rafId = requestAnimationFrame(() => { rafId = null; autoResize(); });
        }
      }
    });
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['style'] });

    const onFontLoad = () => autoResize();
