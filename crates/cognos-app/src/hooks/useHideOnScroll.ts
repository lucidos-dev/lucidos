...
export function useHideOnScroll(headerRef: { current: HTMLElement | null }) {
  useEffect(() => {
    ...
    function onFocusOut(e: FocusEvent) {
      ...
      if (next && isTextInput(next) && !headerRef.current?.contains(next)
    ...
    if (swipeWrapper) {
      observer.observe(swipeWrapper, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });
    }
