...
// `cognos.ui.watchPreferences()` SSE subscription.
//
...
test.describe('SDK iframe theme — live PreferencesChanged update', () => {
  test.beforeAll(() => {
    ...
    // Mirror calendar-light's structure: stylesheet + sdk.js in head,
    // app code in a separate script.js that polls for cognos availability
    // before initializing.
...
`);
    writeFileSync(resolve(APP_DIR, 'script.js'), `
function waitForCognos() {
  if (window.cognos && window.cognos.data) {
    cognos.ui.applyPreferences().then(() => {
      document.getElementById('status').textContent = 'applied';
    });
    cognos.ui.watchPreferences();
  } else {
    setTimeout(waitForCognos, 100);
  }
}
waitForCognos();
`);
  ...
  test('app inside CognOS iframe receives live PreferencesChanged via SSE', async ({ page, request }) => {
    ...
    await page.evaluate((id) => localStorage.setItem('cognos-device-id', id), deviceId);
    ...
    // where CognOS UI subscribes to the same /api/events).
...
<script>
  // Mirror the parent CognOS app — open an EventSource so connection-limit
