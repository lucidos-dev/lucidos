...
test.describe('Message route panel', () => {
  ...
  test('clicking the route badge opens the panel; outside-click closes it', async ({ page }) => {
    ...
    // The frontend always sends x-cognos-device-id (auto-generated in
  ...
  test('engine-origin MessageReceived renders "Engine · Auto-resumed after restart" in popover', async ({ page }) => {
    ...
    try {
      psql([
        `INSERT INTO thread_summaries (thread_id, title, source, last_activity, message_count, is_pinned, has_response, status, section, is_cc, active_children_count, cc_has_changes, cc_requires_restart, cc_is_external_repo) VALUES ('${threadId}', 'Engine origin e2e', 'chat', '${now}', 1, false, true, 'done', 'unread', false, 0, false, false, false)`,
      ...
      await page.addInitScript((tid: string) => {
        localStorage.setItem('cognos-focused-thread', tid);
      }, threadId);
      await navigateToApp(page);
      await assertHealthy(page);
      ...
      await expect(panel).toBeVisible();
      const originSection = panel.locator('.route-section').first();
      await expect(originSection).toContainText('Engine');
      await expect(originSection).toContainText('Auto-resumed after restart');
    } finally {
      psql([
