//! MCP stdio server invoked by CC as `--permission-prompt-tool mcp__cognos_perm__approve`.
...
pub fn run() -> Result<(), BoxError> {
    ...
    let thread_id = std::env::var("COGNOS_THREAD_ID")
        .map_err(|_| "COGNOS_THREAD_ID env var required for permission-prompt server")?;
    ...
    for line in stdin.lock().lines() {
        ...
        let req: JsonRpcRequest = match serde_json::from_str(&line) {
            ...
            Err(e) => {
                ...
                // can't be recovered.
                eprintln!("[cognos-mcp-perm] bad jsonrpc: {}", e);
...
fn handle(
    ...
    let result = match req.method.as_str() {
        "initialize" => Ok(serde_json::json!({
            ...
            "capabilities": { "tools": {} },
            "serverInfo": { "name": "cognos-perm", "version": env!("CARGO_PKG_VERSION") }
        })),
        "tools/list" => Ok(serde_json::json!({
            "tools": [{
                "name": "approve",
                "description": "Surfaces a permission prompt to the CognOS user",
                "inputSchema": {
                    "type": "object",
                    "properties": {
...
mod tests {
    ...
    fn initialize_returns_protocol_version_and_server_info() {
        ...
        assert_eq!(result["serverInfo"]["name"], "cognos-perm");
    }
