...
//!
//!   "cd into a fake worktree dir, run `cognos data write artifacts/test.txt
...
const COGNOS: &str = env!("CARGO_BIN_EXE_cognos");

fn write_ports(workspace: &std::path::Path, port: u16) {
    let cognos = workspace.join(".cognos");
...
fn write_from_worktree_lands_in_parent_workspace_data_dir() {
    ...
    let worktree = workspace.join(".cognos/worktrees/abc123");
    fs::create_dir_all(&worktree).unwrap();

    let src = tmp.path().join("input.txt");
    fs::write(&src, b"hello world").unwrap();

    let out = Command::new(COGNOS)
        .args(["data", "write", "artifacts/ua/test.txt", "--from"])
        .arg(&src)
        .current_dir(&worktree)
        .env_remove("COGNOS_WORKSPACE")
        .output()
        .expect("cognos binary should run");
    assert!(
        out.status.success(),
        "cognos data write failed: stdout={}\nstderr={}",
...
fn write_falls_back_to_env_when_pwd_outside_workspace() {
    ...
    let out = Command::new(COGNOS)
        ...
        .env("COGNOS_WORKSPACE", &workspace)
        .output()
        .expect("cognos binary should run");
    assert!(
        out.status.success(),
...
fn data_path_prints_resolved_path_with_normalization() {
    ...
    let worktree = workspace.join(".cognos/worktrees/foo");
    fs::create_dir_all(&worktree).unwrap();
