//! Scheduler module for CognOS periodic triggers
//!
...
/// Manages all triggers in CognOS
...
impl SchedulerManager {
    ...
    async fn replay_triggers_from_events(&self) {
        ...
        let enabled = triggers.values().filter(|t| t.enabled).count();
        ...
        if count > 0 {
            log!(
                "Replayed {} trigger events → {} triggers ({} enabled)",
                count,
                total,
                enabled
            );
    ...
    /// Register all enabled triggers from in-memory configs with the cron scheduler.
...
async fn run_task_loop(
    ...
    loop {
        ...
        let config = match config {
            ...
            Some(_) => return Ok("trigger disabled".to_string()),
...
async fn check_and_execute_missed(
    ...
    if let Some(missed) = best_missed {
        ...
        let config = match config {
            Some(c) if c.enabled => c,
            _ => return Ok(()), // Trigger deleted or disabled
        };
    ...
    match event_type {
        "TriggerCreated" => {
            if let Ok(config) = TriggerConfig::from_created_payload(payload) {
                let should_register = config.enabled && !config.schedule.is_empty();
                {
        ...
        "TriggerUpdated" => {
            ...
            if let Some(config) = config_snapshot {
                ...
                if config.enabled && !config.schedule.is_empty() {
                    register_and_track(
                        &config,
                        tracked_tasks,
                        engine,
        ...
        "TriggerEnabled" => {
            ...
            if let Some(config) = config_snapshot {
                ...
                crate::log!("[Scheduler] Enabled trigger: {}", trigger_id);
            }
        }
        "TriggerDisabled" => {
            {
            ...
            cancel_tracked_task(tracked_tasks, task_uuid).await;
            crate::log!("[Scheduler] Disabled trigger: {}", trigger_id);
        }
        _ => {}
    }
}
...
mod tests {
    ...
    fn find_matching_script_in_triggers() {
        let dir = std::env::temp_dir().join("cognos_test_find_script_v2");
    ...
    fn find_matching_script_no_run_py() {
        let dir = std::env::temp_dir().join("cognos_test_find_script_no_py");
        let _ = std::fs::remove_dir_all(&dir);
