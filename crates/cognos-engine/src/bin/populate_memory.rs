...
//!
//! Usage: cargo run -p cognos-engine --bin populate_memory [workspace_path]

use chrono::{Duration, Utc};
use cognos_engine::core::EventStore;
use cognos_engine::log;
use cognos_engine::memory::{EmbeddingProvider, FastEmbedProvider, MemorySource, PgVectorIndex};
use cognos_engine::scheduler::NotificationStore;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    log!("\nTo test, run:");
    log!(
        "  COGNOS_WORKSPACE={} cargo run -p cognos-engine",
        workspace_path.display()
    );
