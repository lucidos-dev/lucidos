...
impl PythonRuntime {
    pub fn new(workspace_path: PathBuf) -> Self {
        ...
        let exhaust_path = workspace_path.join(".cognos").join("exhaust");
        fs::create_dir_all(&exhaust_path).ok();

        // Clean up orphaned staging dirs from previous crashed runs
        let staging_root = workspace_path.join(".cognos/staging");
        if staging_root.exists() {
            fs::remove_dir_all(&staging_root).ok();
        }

        let venv_path = workspace_path.join(".cognos/runtime/python/venv");
...
mod tests {
    ...
    async fn test_sandbox_blocks_write_outside_workspace() {
        ...
        let result = runtime
            .execute("open('/etc/cognos_sandbox_test', 'w').write('bad')")
            .await;
        assert!(result.is_err());
    ...
    async fn test_staging_redirects_data_writes() {
        ...
        let staging = ws.join(".cognos/staging/test-run");
    ...
    async fn test_staging_reads_fall_through_to_workspace() {
        ...
        let staging = ws.join(".cognos/staging/test-run-2");
    ...
    async fn test_staging_reads_own_writes() {
        ...
        let staging = ws.join(".cognos/staging/test-run-3");
    ...
    async fn test_staging_non_data_writes_go_to_workspace() {
        ...
        let staging = ws.join(".cognos/staging/test-run-4");
