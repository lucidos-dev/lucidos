...
#[test]
fn build_command_sets_cognos_thread_id_env() {
    let thread_id = uuid::Uuid::new_v4();
    let p = std::path::Path::new("/tmp");
    let cmd = build_command(&test_spawn_args(p, p, thread_id), None);
    let env = collect_envs(&cmd);
    let value = env
        .get(std::ffi::OsStr::new("COGNOS_THREAD_ID"))
        .expect("COGNOS_THREAD_ID env var must be set on the spawned subprocess");
    assert_eq!(value, std::ffi::OsStr::new(&thread_id.to_string()));
}

#[test]
fn build_command_sets_cognos_workspace_env() {
    let thread_id = uuid::Uuid::new_v4();
    let workspace = std::path::Path::new("/some/workspace");
    let worktree = std::path::Path::new("/some/workspace/.cognos/worktrees/abc");
    let cmd = build_command(&test_spawn_args(worktree, workspace, thread_id), None);
    let env = collect_envs(&cmd);
    assert_eq!(
        env.get(std::ffi::OsStr::new("COGNOS_WORKSPACE"))
            .map(|v| v.as_os_str()),
...
fn build_command_prepends_cli_dir_to_path() {
    let thread_id = uuid::Uuid::new_v4();
    let p = std::path::Path::new("/tmp");
    let cli_dir = std::path::Path::new("/opt/cognos/bin");
    ...
    assert!(
        ...
        "PATH {:?} should start with cognos cli dir {:?}",
        path_str,
        cli_dir
    );
}
...
fn build_command_uses_permission_prompt_tool_not_skip_permissions() {
    ...
    let prompt_tool_idx = args
        ...
        .unwrap();
    assert_eq!(args[prompt_tool_idx + 1], "mcp__cognos_perm__approve");
    ...
    assert_eq!(cfg["mcpServers"]["cognos_perm"]["command"], "cognos");
    assert_eq!(
        cfg["mcpServers"]["cognos_perm"]["args"][0],
...
fn build_command_skips_path_injection_when_no_cli_dir() {
    ...
    assert!(
        ...
        "PATH must not be set when cognos CLI binary is missing — \
