...
fn read_engine_version() -> String {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("VERSION");
...
/// Restart the engine by spawning web-dev.sh --engine-only.
/// This rebuilds and restarts only the engine binary, leaving Vite and parent
/// scripts (web-dev.sh, tauri-dev.sh) untouched.
pub(super) async fn restart_engine(State(state): State<AppState>) -> StatusCode {
    let repo_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    let script = repo_root.join("scripts/web-dev.sh");
    let ws = state.workspace_path.to_string_lossy().to_string();
    let log_path = state.workspace_path.join(".cognos/engine.log");
    log!("[Restart] Running web-dev.sh --engine-only -w {}", ws);
    ...
    match cmd.spawn() {
        Ok(_) => StatusCode::OK,
        Err(e) => {
            log!("[Restart] Failed to spawn web-dev.sh: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        }
    }
}

/// List other running CognOS workspaces by calling status.sh --json.
...
pub(super) async fn list_workspaces(State(state): State<AppState>) -> Json<serde_json::Value> {
    ...
    let repo_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    let script = repo_root.join("scripts/status.sh");
    let ws = state.workspace_path.to_string_lossy().to_string();
