...
async fn search_changes_internal(
    ...
    let (pending, applied) = tokio::join!(
        crate::core::changes::list_pending(pool),
        crate::core::changes::list_recently_applied(pool, 15, None),
    );
    let pending = pending.map_err(|e| format!("Changes query failed: {}", e))?;
    let applied = applied.map_err(|e| format!("Changes query failed: {}", e))?;
