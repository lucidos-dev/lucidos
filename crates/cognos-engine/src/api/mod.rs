...
pub struct ChatRequest {
    ...
    ///
    /// The legacy field name `"sender"` and its legacy values `"user"` /
    /// `"system"` continue to deserialize for HTTP clients posting the
    /// legacy wire format: `"user"` → Human, `"system"` → Agent. New
    /// callers should send `"mode": "human"` / `"agent"` / `"engine"`.
    #[serde(
        alias = "sender",
        deserialize_with = "crate::engine::thread_events::deserialize_actor_mode_with_legacy_aliases"
    )]
    ...
    /// Required when `sender == "system"`: the thread that is spawning this
    /// new thread (e.g. the CC session whose `spawn-thread` skill is making
    /// this call). Must be `null` when `sender == "user"`. The `spawn-thread`
    /// skill reads `$COGNOS_THREAD_ID` and forwards it here.
    #[serde(default)]
    ...
    /// `spawn-thread` skill. Allowed only when `sender == "system"`. Lets
    /// audits trace exactly which step in the parent thread caused this spawn.
    #[serde(default)]
...
pub fn create_router(
    ...
    let router = if let Ok(vite_url) = std::env::var("COGNOS_DEV_PROXY") {
