...
pub async fn start_workspace(
    ...
    let ports_file = canonical.join(".cognos").join("ports");
    ...
    // Find scripts dir and start workspace via web-dev.sh in background
    let scripts_dir = backup::find_scripts_dir().map_err(|e| {
        json_error(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Cannot find scripts: {e}"),
        )
    })?;
    let web_dev = scripts_dir.join("web-dev.sh");
    ...
    // Determine protocol
    let proto = if std::env::var("COGNOS_TLS_CERT").is_ok() {
        "https"
    } else {
        "http"
    };
