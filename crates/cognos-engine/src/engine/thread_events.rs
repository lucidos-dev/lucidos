...
fn is_default_reason(r: &SessionEndReason) -> bool {
    *r == SessionEndReason::Completed
}

/// Backward-compat default for `MessageReceived.mode` on DB rows persisted
/// before any mode/sender field existed. New emissions MUST set `mode`
/// explicitly — the API layer enforces this on incoming requests.
fn default_mode_human() -> ActorMode {
    ActorMode::Human
}

/// Custom deserializer for `MessageReceived.mode` that also accepts the legacy
/// string values `"user"` and `"system"` from the pre-rename `sender` field.
/// Combined with `#[serde(alias = "sender")]`, this lets historical DB rows
/// continue to deserialize cleanly: the alias matches the old field name and
/// this fn maps the value to the new enum (`user → Human`, `system → Agent`).
///
/// Also reused by the HTTP `ChatRequest` deserializer so existing clients that
/// still post `"sender": "user"` / `"sender": "system"` keep working.
pub fn deserialize_actor_mode_with_legacy_aliases<'de, D: serde::Deserializer<'de>>(
    d: D,
) -> Result<ActorMode, D::Error> {
    let s = String::deserialize(d)?;
    Ok(match s.as_str() {
        "user" => ActorMode::Human,    // legacy Initiator::User
        "system" => ActorMode::Agent,  // legacy Initiator::System
        "human" => ActorMode::Human,
        "agent" => ActorMode::Agent,
        "engine" => ActorMode::Engine,
        other => return Err(serde::de::Error::custom(format!("unknown mode '{}'", other))),
    })
}
...
/// synthesizes Device / ParentThread from the legacy `device_id` /
/// `parent_thread_id` fields when origin is missing.
///
/// Invariants enforced at construction in `engine/chat/events.rs`:
/// - `Device | Api`             ⇒ `mode == ActorMode::Human`
/// - `Workspace { mode, .. }`   ⇒ `mode == carried mode` (Human or Agent)
/// - `ParentThread { mode, .. }`⇒ `mode == carried mode` (Human, Agent, or Engine)
/// - `Engine { .. }`            ⇒ `mode == ActorMode::Engine`
///
/// Workspace headers are display hints only — they are user-controllable
...
pub enum MessageOrigin {
    ...
    /// Mode = Human. HTTP request without a `device_id` and without
    /// the `X-Cognos-Workspace` header.
    Api {
        #[serde(default, skip_serializing_if = "Option::is_none")]
        user_agent: Option<String>,
    },
    /// Mode = Human or Agent (carried in `mode`). HTTP request from another
    /// CognOS workspace, identified by the `X-Cognos-Workspace` header.
    Workspace {
        workspace: String,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        ...
        /// cross-workspace calls must set this explicitly via the `X-Cognos-Mode`
        /// header. Read by `MessageOrigin::mode()` to derive the request's `ActorMode`.
        #[serde(default = "default_workspace_mode")]
        mode: ActorMode,
    },
    /// Mode = Human, Agent, or Engine (carried in `mode`). Thread was spawned
    /// by another thread.
    ParentThread {
        thread_id: uuid::Uuid,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        ...
        /// Defaults to Agent because all pre-existing parent-thread spawns were
        /// LLM-driven; engine-initiated parent spawns must set this explicitly.
        /// Read by `MessageOrigin::mode()` to derive the request's `ActorMode`.
        #[serde(default = "default_parent_thread_mode")]
        mode: ActorMode,
    },
...
fn default_parent_thread_mode() -> ActorMode {
    ActorMode::Agent
}

impl MessageOrigin {
    /// Derive the actor mode from the origin variant. For `Workspace` and
    /// `ParentThread` this reads the carried field; for the rest the mode is
    /// intrinsic to the variant.
    pub fn mode(&self) -> ActorMode {
        match self {
            Self::Device { .. } | Self::Api { .. } => ActorMode::Human,
            Self::Workspace { mode, .. } | Self::ParentThread { mode, .. } => *mode,
...
pub enum ThreadEvent {
    ...
    MessageReceived {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        spawning_event_id: Option<uuid::Uuid>,
        /// Semantic mode of the actor that emitted this message. Replaces the
        /// historical `sender` field (User/System); kept under that alias so DB
        /// rows persisted before the rename deserialize cleanly via
        /// `deserialize_mode_with_legacy_aliases` (User → Human, System → Agent
        /// — matches prior implicit semantics). Required for new emissions
        /// (enforced at the API layer). The serde default exists only to
        /// replay old DB rows persisted before any mode/sender field existed.
        #[serde(
            default = "default_mode_human",
            alias = "sender",
            deserialize_with = "deserialize_actor_mode_with_legacy_aliases"
        )]
        mode: ActorMode,
    ...
    SessionEnded {
        /// Why the session ended. Defaults to Completed for old DB rows.
        #[serde(
            default = "default_session_ended_reason",
            skip_serializing_if = "is_default_reason"
        )]
    ...
    // Mid-flight injection — user sent a correction while the agentic loop was running
    UserPromptInjected {
        text: String,
    },
...
mod tests {
    ...
    #[test]
    fn message_origin_parent_thread_defaults_mode_to_agent_when_missing() {
        let json = r#"{
            "kind": "parent_thread",
        ...
        let parsed: MessageOrigin = serde_json::from_str(json).unwrap();
        match parsed {
            MessageOrigin::ParentThread { mode, .. } => assert_eq!(mode, ActorMode::Agent),
    ...
    #[test]
    fn message_origin_parent_thread_round_trips_with_explicit_engine_mode() {
        let original = MessageOrigin::ParentThread {
    ...
    fn message_origin_mode_derives_human_for_device_and_api() {
        ...
        let api = MessageOrigin::Api { user_agent: None };
    ...
    fn session_ended_reason_serialization() {
        // Default reason Completed is skipped in serialization
        let event = ThreadEvent::SessionEnded {
            reason: SessionEndReason::Completed,
        };
        let serialized = serde_json::to_value(&event).unwrap();
        ...
        assert!(
            ...
            "default reason 'completed' should be skipped"
        );

        // Non-default reason is included
        let event2 = ThreadEvent::SessionEnded {
            reason: SessionEndReason::UserEnded,
        };
        let serialized2 = serde_json::to_value(&event2).unwrap();
        assert_eq!(serialized2["type"], "SessionEnded");
        assert_eq!(serialized2["reason"], "user_ended");

        // Old DB format without reason deserializes with default
        let old: ThreadEvent = serde_json::from_str(r#"{"type":"SessionEnded"}"#).unwrap();
        match old {
            ThreadEvent::SessionEnded { reason } => assert_eq!(reason, SessionEndReason::Completed),
            _ => panic!("wrong variant"),
        }

        // New format with reason
        let new: ThreadEvent =
            serde_json::from_str(r#"{"type":"SessionEnded","reason":"changes_applied"}"#).unwrap();
        match new {
            ThreadEvent::SessionEnded { reason } => {
                assert_eq!(reason, SessionEndReason::ChangesApplied)
            }
            _ => panic!("wrong variant"),
        }
    }
    ...
    // ---- Legacy `sender` field deserialization for MessageReceived ----
    //
    // Pre-rename DB rows persisted `MessageReceived` with a `sender` field
    // valued `"user"` or `"system"`. The field is `mode` of `ActorMode`; old
    // DB rows persisted `sender` of `"user"`/`"system"`. The serde alias +
    // custom deserializer must keep those rows loading: User → Human,
    // System → Agent.

    #[test]
    fn message_received_legacy_sender_user_deserializes_to_mode_human() {
        let json = r#"{"type":"MessageReceived","text":"hi","sender":"user"}"#;
        let parsed: ThreadEvent = serde_json::from_str(json).unwrap();
        match parsed {
    ...
    #[test]
    fn message_received_legacy_sender_system_deserializes_to_mode_agent() {
        let json = r#"{"type":"MessageReceived","text":"hi","sender":"system"}"#;
        let parsed: ThreadEvent = serde_json::from_str(json).unwrap();
        match parsed {
            ThreadEvent::MessageReceived { mode, .. } => assert_eq!(mode, ActorMode::Agent),
            other => panic!("expected MessageReceived, got {:?}", other),
        }
    }

    #[test]
    fn message_received_new_mode_field_deserializes() {
    ...
    #[test]
    fn message_received_no_mode_or_sender_defaults_to_human() {
