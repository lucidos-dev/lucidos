...
/// Regression test: domain events emitted via `cognos.events.emit()` must
/// be broadcast on SSE with the inner event_type as the wire `type`, so
/// that frontend listeners (`cognos.sse.on('SlidePresenterState', ...)`)
...
fn domain_event_sse_json_uses_inner_event_type() {
    ...
    assert_eq!(
        ...
        "Wire `type` must be the inner event_type, not the wrapper 'DomainEvent', \
             so frontend `cognos.sse.on('SlidePresenterState', ...)` fires"
    );
    assert_eq!(
...
/// Regression test: SessionEnded must be the last event emitted for a CC thread.
/// Previously, SessionEnded was emitted inside the cancel handler before cleanup
/// (change proposal, worktree removal), causing ChangeProposed to appear after
/// SessionEnded. The frontend then showed "Idle" instead of "Done".
#[test]
fn session_ended_must_follow_change_proposed_in_event_stream() {
    let (tx, mut rx) = broadcast::channel::<EmittedEvent>(16);
    let tid = Uuid::new_v4();

    // Simulate the correct event ordering from run_direct_agent:
    // ResponseGenerated → CodingAgentIdled → ChangeProposed → SessionEnded
    let events = vec![
    ...
    // Drain and verify SessionEnded is the last event
    ...
    assert_eq!(
        ...
        "SessionEnded",
        "SessionEnded must be the final event — ChangeProposed must come before it"
    );
}
...
async fn test_cc_child_session_ended_without_idle_sends_callback() {
    ...
    // (simulates crash/shutdown/user-ended scenario)
    bus.emit(BusEvent::Thread {
        thread_id: child_id,
...
async fn test_cc_child_no_duplicate_callback_after_idle_then_session_ended() {
    ...
    // Now SessionEnded fires (normal lifecycle after idle)
    bus.emit(BusEvent::Thread {
        thread_id: child_id,
        event: ThreadEvent::SessionEnded {
            reason: SessionEndReason::Completed,
...
/// Regression: SessionEnded with StaleResume must NOT change thread status.
/// Stale resume is an internal retry — the user's message is still being processed.
/// Without this fix, the thread would go idle/waiting while a new CC session is running.
#[tokio::test]
async fn test_stale_resume_session_ended_preserves_running_status() {
    let (pool, db_name) = setup_test_db().await;
    let (bus, _callback_rx) = EventBus::new(pool.clone());

    let thread_id = Uuid::new_v4();

    // 1. MessageReceived → status = running
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::MessageReceived {
            text: "fix it".into(),
    ...
    // 2. SessionStarted (stale session)
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::SessionStarted {
    ...
    // 3. SessionEnded with StaleResume — must NOT change status
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::SessionEnded {
    ...
    assert_eq!(
        status, "running",
        "StaleResume should not change thread status"
    );

    // 4. New SessionStarted (fresh retry) — still running
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::SessionStarted {
    ...
    assert_eq!(
        status, "running",
        "Thread should remain running through stale resume retry"
    );

    // 5. Normal SessionEnded SHOULD change status to idle
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::SessionEnded {
            reason: SessionEndReason::Completed,
    ...
    assert_eq!(
        status, "idle",
        "Normal SessionEnded should still transition to idle"
    );
...
async fn test_recursion_guard_allows_shallow_threads() {
    ...
    let result = crate::engine::CognosEngine::check_thread_recursion_guard(&pool, root).await;
...
async fn test_recursion_guard_enforces_max_depth() {
    ...
    let result = crate::engine::CognosEngine::check_thread_recursion_guard(&pool, deepest).await;
    ...
    let result2 =
        crate::engine::CognosEngine::check_thread_recursion_guard(&pool, second_to_last).await;
    assert!(
        result2.is_ok(),
...
async fn test_recursion_guard_unknown_parent_treated_as_root() {
    ...
    let result = crate::engine::CognosEngine::check_thread_recursion_guard(&pool, unknown).await;
...
async fn test_recursion_guard_graceful_error_message() {
    ...
    let result = crate::engine::CognosEngine::check_thread_recursion_guard(&pool, parent).await;
...
async fn test_recursion_guard_parallel_children_within_limit() {
    ...
    for i in 0..crate::engine::chat::MAX_CHILDREN_PER_THREAD {
        ...
        let result = crate::engine::CognosEngine::check_thread_recursion_guard(&pool, child).await;
...
async fn test_recursion_guard_children_limit_per_parent_not_global() {
    ...
    let result =
        crate::engine::CognosEngine::check_thread_recursion_guard(&pool, first_child).await;
    assert!(
        result.is_ok(),
...
async fn session_ended_sets_has_response_true() {
    ...
    // Session ends normally (without prior idle — e.g. crash or force cancel)
    bus.emit(BusEvent::Thread {
        thread_id,
        event: ThreadEvent::SessionEnded {
            reason: SessionEndReason::Completed,
    ...
    assert!(
        has_response,
        "SessionEnded (non-StaleResume) should set has_response = true"
    );
