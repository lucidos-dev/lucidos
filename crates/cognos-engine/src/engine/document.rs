...
pub(super) fn extract_text_with_ocr(pdf_path: &Path) -> Result<String, String> {
    ...
    let temp_dir = std::env::temp_dir().join(format!("cognos_ocr_{}", uuid::Uuid::new_v4()));
...
pub(super) fn ocr_image_with_paddle(image_path: &Path) -> Result<String, String> {
    ...
    // Get models directory (in .cognos/models/)
    let models_dir = std::env::var("COGNOS_WORKSPACE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."))
        .join(".cognos/models/ocr");
