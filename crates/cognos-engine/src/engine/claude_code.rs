use super::{CcCommandsResult, CognosEngine};
...
pub(crate) const WORKTREE_WORKSPACE_MARKER: &str = ".cognos-workspace";
...
impl CodingAgent for CognosEngine {
    ...
    fn spawn_hardening(
        ...
        self.spawn_hardening_session(thread_id, worktree_path, branch_name, auto_apply_change_id);
    }
    ...
    fn spawn_merge_session(&self, thread_id: Uuid, change_id: Uuid, description: &str) {
        ...
        Self::spawn_cc_task_guarded(engine.clone(), thread_id, async move {
            let change = match crate::core::changes::get_by_id(engine.pool(), change_id).await {
                Ok(Some(c)) => c,
                Ok(None) => {
                    log!(
                ...
                Err(e) => {
                    log!(
                        "[MergeConflict] Failed to load change {} for spawn_merge_session: {}",
                        change_id,
                        e
                    );
                    return;
...
impl CognosEngine {
    ...
    pub async fn cancel_agent(
        ...
        if let Some(tid) = thread_id {
            if let Some(session) = guard.get_mut(&tid) {
                ...
                drop(guard);
                // No live session — try handling as a stale waiting session from a previous engine run
                self.end_stale_waiting_session(tid, auto_apply, discard)
                    .await
            }
        } else {
            if guard.is_empty() {
    ...
    pub async fn interrupt_agent(
        ...
        if let Some(tid) = thread_id {
            if let Some(session) = guard.get(&tid) {
                ...
                Err("No Claude Code session found for this thread".into())
            }
        } else {
            if guard.is_empty() {
                return Err("No Claude Code process is running".into());
    ...
    /// Spawn a CC session to run hardening on an existing worktree.
    /// If `auto_apply_change_id` is provided, the change will be auto-applied after hardening
    /// completes, with ChangeApplyFailed emitted on failure.
        ...
        Self::spawn_cc_task_guarded(engine.clone(), thread_id, async move {
            ...
            match result {
                Ok(_) => {
                    ...
                    // Mark as hardened before auto-apply so apply_change doesn't
                    // re-enter the unhardened path and spawn another hardening → infinite loop.
                    if let Err(e) =
                        crate::core::changes::set_hardened(engine.pool(), change_id).await
                    {
                        log!(
                            "[ClaudeCode] Failed to set hardened flag for change {}: {}",
                            change_id,
                            e
                        );
                    }
                    match engine.apply_change(change_id, None).await {
                        Ok(r) => {
                            log!(
    ...
    pub async fn end_cc_session_for_thread(
        ...
        self.cancel_agent(false, true, Some(thread_id)).await
    }
    ...
    pub async fn discard_cc_changes(
        ...
        let wt = {
            ...
            if let Some(session) = guard.get(&thread_id) {
                ...
                return self.end_stale_waiting_session(thread_id, false, true).await;
            }
        };
        ...
        let emit_result = self
            ...
            .emit(crate::engine::event_bus::BusEvent::Thread {
                ...
                event: crate::engine::thread_events::ThreadEvent::MessageReceived {
                    ...
                    // CC sessions emit model/effort via CodingAgentSettingsChanged.
                    model: None,
            ...
            match result {
                Ok(ref res) => {
                    if res.proposed_change {
                        if res.auto_apply {
                            let pending = changes::list_pending(engine.pool())
                                .await
                                .unwrap_or_default();
                            if let Some(change) =
...
#[cfg(test)]
mod tests {
    use super::{cc_allowed_tools, DEFAULT_CC_ALLOWED_TOOLS};
