...
/// Save user-attached images to .cognos/tmp/images/ so the LLM can reference them by path.
/// Returns a list of relative paths (e.g., ".cognos/tmp/images/20260317-143052-0.jpg").
pub(super) fn save_images_to_tmp(
    ...
    let images_dir = workspace_path.join(".cognos/tmp/images");
    ...
    for (i, img) in images.iter().enumerate() {
        ...
        let rel_path = format!(".cognos/tmp/images/{}", filename);
    ...
    if !paths.is_empty() {
        crate::log!(
            "[Image] Saved {} image(s) to .cognos/tmp/images/",
            paths.len()
        );
    }
