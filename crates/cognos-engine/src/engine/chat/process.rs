    ...
    async fn process_message_with_steps_internal(
        ...
        if use_claude_code == Some(true) && !is_new_thread && !user_message.is_empty() {
            if let Some(pending_tool_use_id) =
                crate::engine::agent_question::lookup_pending_question_tool_use_id(
                    self.pool(),
                    thread_id,
                )
                .await
            {
                    ...
                    .await
                {
                    ...
                    AnswerResult::ResumeFailed(msg) => {
                        log!(
                            "[Chat] Free-form answer resume failed for thread {}: {}",
                            thread_id,
                            msg
                        );
                        emit_routing_failure(&self.event_bus, thread_id, &msg).await?;
        ...
        // Auto-save images to .cognos/tmp/images/ so the LLM can reference them by path
        ...
        // Direct Claude Code mode — run CC in the same thread (no spawn)
        if use_claude_code == Some(true) {
            ...
            // SessionEnded was emitted to prevent re-resume. Retry with fresh session.
...
- The system automatically handles daylight saving time adjustments."#,
        ...
        let system_prompt = format!(
            r#"You are CognOS, a personal cognitive assistant running in the "{workspace_name}" workspace. You help users organize their life and work through natural conversation.
...
The workspace root has two top-level areas:
  ...
  .cognos/                    ← Ephemeral, NOT under data/, NOT git-tracked
    tmp/                      ← Temp files from http_request (e.g., .cognos/tmp/oura_data.json)
...
- .cognos/ paths are relative to workspace root — use ".cognos/tmp/file.json", NEVER "data/.cognos/tmp/file.json"
- In Python scripts (run_python), cwd is the workspace root. Use open('.cognos/tmp/file.json') for temp files, open('data/artifacts/file.md') for artifacts.
...
- Every script the engine spawns gets the `cognos` CLI on PATH and `COGNOS_WORKSPACE` set automatically.
- For data writes use `cognos data write artifacts/<name>.json --from /tmp/x` (or `--from -` for stdin), NOT raw HTTP requests to the engine.
- For domain events use `cognos events emit <Type> --summary "..." --payload '{...}'` and `cognos events query --type <Type> --limit N`.
- See knowhow/cognos-cli.md for the full reference.
...
- NEVER use browser_open on your own App UIs, artifacts, or any CognOS internal files
...
- App UIs access the platform via the CognOS SDK (<script src="/api/v1/sdk.js">). Key SDK methods: cognos.data.read/write/delete/list (file CRUD), cognos.events.emit/query (domain events), cognos.preferences.get/set, cognos.ui.applyPreferences() (theme/font), cognos.ui.navigate(target, params), cognos.sse.on(type, cb) (real-time event stream).
...
You have two tools for delegating work to child threads:
- run_claude: Start a Claude Code session for code tasks (creates worktree, proposes changes)
- run_thread: Start a CognOS thread for non-code tasks (research, analysis, drafting)
Each child thread runs independently. When it completes, a callback resumes this thread with its result. NOTE: this callback only works for SAME-workspace children spawned via these tools. POSTs to another workspace's /api/chat/stream are fire-and-forget — see the cross-workspace knowhow.
...
VERIFICATION: Before saying "done", check that write_file returned "Created:" or "Updated:" — if not, you didn't actually do it!"#;
        ...
        // Tell the LLM where CognOS is running
        let api_port = std::env::var("COGNOS_API_PORT").unwrap_or_else(|_| "3000".to_string());
        ...
        let system_prompt = format!("{}\n\nThe CognOS client the user is talking to you from is at {}. To see App UIs, use capture_app_ui — never browser_open.",
        ...
        // Add system docs summaries (engine-shipped reference, never overrideable).
        let system_docs_summaries = self
            .system_docs_dir()
            .map(crate::core::SystemDocsStore::load_summaries)
            .unwrap_or_default();
        let system_docs_section = if !system_docs_summaries.is_empty() {
            let mut section = String::from("\n\n## System Docs\n\n\
                Authoritative reference shipped with the CognOS engine. Use \
                `load_knowhow` with the prefixed id (e.g. `system-docs/<id>`) to load full content.\n\n");
            for sd in &system_docs_summaries {
                section.push_str(&format!(
                    "- **{}** (id: `system-docs/{}`): {}\n",
        ...
        let system_prompt = format!(
            "{}{}{}{}{}",
            system_prompt, apps_section, intents_section, knowhow_section, system_docs_section
        );
...
[END URL CONTEXT]",
        ...
        // Emit context sections for the frontend context viewer
        {
            let tool_names: Vec<&str> = tools.iter().map(|t| t.name.as_str()).collect();
            let tool_names_str = tool_names.join(", ");

            let mut sections = Vec::new();
            let add = |sections: &mut Vec<ContextSection>, name: &str, content: &str| {
                if !content.is_empty() {
            ...
            add(&mut sections, "System Instructions", &system_prompt);
            add(&mut sections, "User Profile", &profile_context);
            add(&mut sections, "Long-term Memory", &memory_context);
            add(&mut sections, "Conversation History", &history_context);
            add(&mut sections, "File List", &file_list_context);
            add(&mut sections, "Credentials", &credentials_context);
            add(&mut sections, "Email Accounts", &email_accounts_context);
            add(&mut sections, "OAuth", &oauth_context);
            add(&mut sections, "App Context", &app_context_section);
            add(&mut sections, "File Context", &file_context_section);
            add(&mut sections, "URL Context", &url_context_section);
            add(&mut sections, "User Message", user_message);
            add(&mut sections, "Tools", &tool_names_str);
