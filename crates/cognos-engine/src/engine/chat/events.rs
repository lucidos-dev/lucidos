...
pub(super) fn make_message_received(
    user_message: &str,
...
fn synthesize_legacy_origin(
    ...
    match mode {
        ActorMode::Human => device_id.map(|id| MessageOrigin::Device {
            ...
            label: device_name.unwrap_or("Unknown device").to_string(),
        }),
        ActorMode::Agent => parent_thread_id.map(|id| MessageOrigin::ParentThread {
            thread_id: id,
            title: None,
            spawning_event_id,
            mode: ActorMode::Agent,
        }),
        ActorMode::Engine => parent_thread_id.map(|id| MessageOrigin::ParentThread {
            thread_id: id,
            title: None,
            spawning_event_id,
...
mod origin_invariants {
    ...
    #[test]
    fn api_origin_with_agent_mode_rejected() {
        ...
        assert!(res.is_err(), "Api origin must reject non-Human mode");
    }
    ...
    #[test]
    fn parent_thread_origin_with_matching_agent_mode_ok() {
        let origin = MessageOrigin::ParentThread {
    ...
    #[test]
    fn parent_thread_origin_with_mismatched_mode_rejected() {
        let origin = MessageOrigin::ParentThread {
            thread_id: Uuid::new_v4(),
            title: None,
            spawning_event_id: None,
