...
impl CognosEngine {
    pub(crate) async fn execute_http_tool(
        ...
        // Handle temp_path - save to .cognos/tmp/ (not git-tracked)
        let saved_temp = if let Some(path) = temp_path {
            if (200..300).contains(&status) {
                let tmp_dir = self.workspace_path.join(".cognos").join("tmp");
        ...
        // Handle output_path - save to artifacts/ (git-tracked)
        // Resolve through resolve_data_path for consistent path normalization
