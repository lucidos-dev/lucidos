...
fn read_only_reason(data_path: &str) -> Option<&'static str> {
    crate::core::is_system_doc_path(data_path)
        .then_some("system docs are read-only (shipped with the engine)")
}
...
mod tests {
    ...
    #[test]
    fn read_only_reason_blocks_system_docs() {
        assert!(read_only_reason("system-docs/best-practices.md").is_some());
        assert!(read_only_reason("system-docs/scripts/list.sh").is_some());
    }

    #[test]
    fn read_only_reason_allows_user_paths() {
        assert!(read_only_reason("artifacts/notes.md").is_none());
        assert!(read_only_reason("knowhow/cognos/best-practices.md").is_none());
