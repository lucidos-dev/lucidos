...
use crate::engine::{AgentUserInput, CognosEngine};
use std::path::Path;
use std::sync::Arc;
...
impl CognosEngine {
        ...
        let (worktree_path, branch_name, repo_root, idle_notify, msg_tx, last_event_at) = {
            ...
            match guard.get_mut(&thread_id) {
                ...
                _ => {
                    ...
                    return self.end_stale_waiting_session(thread_id, true, false).await;
                }
            }
        };
    ...
    async fn apply_now_inner(
        ...
        if !branch_is_hardened(&self.pool, repo_root, branch_name).await {
            self.request_hardening_in_session(thread_id, msg_tx, actor.clone())
                .await?;
            ...
            self.send_and_wait(
                ...
                `cargo test -p cognos-engine` and `cd crates/cognos-app && npm test`. \
    ...
    pub(crate) async fn apply_now_success(
        ...
        if let Err(e) = changes::set_merge_shas(&self.pool, change_id, pre_sha, post_sha).await {
            log!(
                "[ApplyNow] Failed to store merge SHAs for {}: {}",
                change_id,
                e
            );
        }
