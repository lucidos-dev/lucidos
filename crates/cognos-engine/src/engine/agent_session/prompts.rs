/// Workspace context preamble shared by all CognOS-repo CC system prompts.
pub(super) fn workspace_preamble(workspace_name: &str) -> String {
    format!(
        "WORKSPACE: You were spawned by the \"{workspace_name}\" CognOS workspace. \
...
const PROCESS_SAFETY_RULE: &str = "\n\n\
    PROCESS SAFETY: Multiple CognOS workspaces run concurrently. NEVER use \
    `pkill -f cognos-engine`, `killall cognos-engine`, or any broad process kill pattern — \
    ...
    To kill a specific engine: `kill $(cat <workspace>/.cognos/engine.pid)`.";
...
pub(super) fn worktree_system_prompt(branch_name: &str, workspace_name: &str) -> String {
    format!(
        "{preamble}\n\n\
         ...
         Your working directory is a complete copy of the CognOS repository. Your changes are \
         ...
         (`cargo build -p cognos-engine`), start the e2e workspace from your worktree \
         ...
         the test suites for the layers you touched (`cargo test -p cognos-engine` for Rust, \
         `cd crates/cognos-app && npm test` for TypeScript). If you skip this, the user pays \
         ...
         simply say goodbye and finish your response — the CognOS engine manages your lifecycle. \
...
pub(super) fn external_repo_recovery_system_prompt(repo_name: &str, branch_name: &str) -> String {
    format!(
        "RECOVERED SESSION: You are running in a worktree on branch `{branch}` of the \"{repo}\" \
         repository that was orphaned when the CognOS engine restarted. A previous Claude Code \
...
pub(super) fn recovery_system_prompt(branch_name: &str, workspace_name: &str) -> String {
    format!(
        "{preamble}\n\n\
         RECOVERED SESSION: The CognOS engine restarted while you were working on branch \
...
pub(crate) fn build_merge_prompt(
    ...
    prompt.push_str(&format!(
        ...
        4. Run `cargo test -p cognos-engine` and `cd crates/cognos-app && npm test` to verify\n\
