...
/// Return the persistent directory for CC worktrees: `<workspace>/.cognos/worktrees/`.
/// Creates the directory if it doesn't exist.
pub(crate) fn worktrees_dir(workspace_path: &Path) -> PathBuf {
    let dir = workspace_path.join(".cognos/worktrees");
...
/// A registered repo is "external" only if its path differs from the engine's
/// own dev repo. Aliases that point at the dev path (e.g. "Lucidos" during the
/// cognos→lucidos rebrand) must stay internal — otherwise the propose-on-idle
/// path silently skips them and the Apply panel never shows. Canonicalize both
/// sides so trailing slashes / symlinks don't flip the decision; fall back to
/// raw equality if either path can't be canonicalized (test-only paths, repos
/// missing on disk).
...
pub(crate) fn files_require_restart(files: &[String]) -> bool {
    files.iter().any(|f| {
        ...
        // packages/cognos-sdk → /api/v1/sdk.js. The bundle is rebuilt by
        // `web-dev.sh -b` on engine restart; without restart the previously-
        // built dist/sdk.js keeps being served.
        let is_sdk_bundle_source = f.starts_with("packages/cognos-sdk/") && !is_test_or_doc;
        // include_str!'d into the engine binary and served at /api/v1/sdk-iframe.*
        let is_engine_bundled_asset = f == "crates/cognos-engine/src/api/sdk_iframe.css"
            || f == "crates/cognos-engine/src/api/sdk_iframe_audio.js";
...
/// HTTP endpoint that the `cognos hardened mark` CLI POSTs to from the
/// `mark-harden.sh` hook.
...
pub(super) async fn auto_commit_safe_files_if_dirty(repo_root: &Path) -> bool {
    ...
    // Auto-commit harmless dirty files that shouldn't block merging CognOS changes
...
pub(super) async fn git_cmd(args: &[&str], dir: &Path) -> Result<std::process::Output, String> {
...
pub(crate) async fn recover_no_commits_branch(
    ...
    let wt_path = match git_cmd(&["worktree", "list", "--porcelain"], repo_root).await {
