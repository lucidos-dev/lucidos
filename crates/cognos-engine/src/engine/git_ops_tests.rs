...
fn files_require_restart_ignores_tests_and_docs() {
    assert!(!files_require_restart(&[
        "crates/cognos-engine/tests/api_e2e.rs".into()
    ]));
    assert!(!files_require_restart(&["README.md".into()]));
    assert!(!files_require_restart(&[
        "crates/cognos-app/src/App.tsx".into()
    ]));
    assert!(!files_require_restart(&[
        "crates/cognos-app/src/global.css".into()
    ]));
    // SDK docs and SDK tests don't affect the bundle
    assert!(!files_require_restart(&[
        "packages/cognos-sdk/README.md".into()
    ]));
    assert!(!files_require_restart(&[
        "packages/cognos-sdk/tests/preferences.test.ts".into()
    ]));
    assert!(!files_require_restart(&[]));
}
...
fn files_require_restart_for_sdk_bundle_sources() {
    // packages/cognos-sdk/src — TS bundled into /api/v1/sdk.js
    assert!(files_require_restart(&[
        "packages/cognos-sdk/src/preferences.ts".into()
    ]));
    assert!(files_require_restart(&[
        "packages/cognos-sdk/src/index.ts".into()
    ]));
    // packages/cognos-sdk root — build script + tsconfig + package.json
    assert!(files_require_restart(&[
        "packages/cognos-sdk/build.mjs".into()
    ]));
    assert!(files_require_restart(&[
        "packages/cognos-sdk/tsconfig.json".into()
    ]));
}
...
fn files_require_restart_for_engine_bundled_iframe_assets() {
    assert!(files_require_restart(&[
        "crates/cognos-engine/src/api/sdk_iframe.css".into()
    ]));
    assert!(files_require_restart(&[
        "crates/cognos-engine/src/api/sdk_iframe_audio.js".into()
    ]));
}
...
// A registered repo whose path resolves to the dev repo path (e.g. "Lucidos"
// pointing at the cognos checkout during the rebrand) must NOT be flagged as
// external — that gate silently disables the Apply panel for any work done
// in that thread.
#[test]
fn repo_at_dev_root_is_not_external() {
    let dev = std::path::PathBuf::from("/Users/me/IdeaProjects/cognos");
    let lucidos_alias = std::path::PathBuf::from("/Users/me/IdeaProjects/cognos");
    assert!(!is_external_repo_path(&lucidos_alias, &dev));
}
...
fn trailing_slash_does_not_make_repo_external() {
    let dev = std::path::PathBuf::from("/Users/me/IdeaProjects/cognos");
    let with_slash = std::path::PathBuf::from("/Users/me/IdeaProjects/cognos/");
