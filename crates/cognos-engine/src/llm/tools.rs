...
/// Tool for navigating the CognOS UI to a specific panel, app, or file.
pub fn get_navigate_ui_tool() -> ToolDefinition {
    ToolDefinition {
        name: tn::NAVIGATE_UI.to_string(),
        description: "Navigate the CognOS UI to a specific panel, app, app UI, file, thread, or form. Use this when the user asks to open, show, or go to something in the interface — e.g. \"open settings\", \"show me the habit tracker\", \"go to triggers\", \"open my budget file\", \"go to that thread\".".to_string(),
        parameters: json!({
            "type": "object",
            "properties": {
                "target": {
...
pub fn get_default_tools() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            parameters: json!({
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Relative path under data/ (e.g., artifacts/notes.md, apps/my-app/knowhow/api-reference.md, knowhow/domain.md). For knowhow/ paths, if the file already exists in shared (~/.cognos/knowhow/) but not locally, the write updates the shared copy."
                    },
                    "content": {
                        "type": "string",
        ...
        ToolDefinition {
            ...
            parameters: json!({
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Relative path under data/ (e.g., artifacts/notes.md, artifacts/deck.slides, apps/my-app/knowhow/api-reference.md, knowhow/domain.md). For knowhow/ paths, if the file already exists in shared (~/.cognos/knowhow/) but not locally, the edit updates the shared copy."
                    },
                    "old_string": {
                        "type": "string",
        ...
        ToolDefinition {
            ...
            description: "Make an HTTP request to fetch data from an API. Use temp_path for raw data (.cognos/tmp/, not git-tracked). Use output_path only for final artifacts (auto-committed).".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "method": {
                    ...
                    "temp_path": {
                        "type": "string",
                        "description": "Filename to save in .cognos/tmp/ (not git-tracked). Just the filename, e.g. 'google_doc.json' — NOT '.cognos/tmp/google_doc.json'."
                    },
                    "output_path": {
                        "type": "string",
                        "description": "Relative path under data/ to save response (e.g., artifacts/imported/oura/sleep.json). Git committed automatically."
                    }
                },
                "required": ["method", "url"]
            }),
        ...
        ToolDefinition {
            ...
            description: "Import a file from the local filesystem into the artifacts directory. The file will be copied, committed to git, and indexed for search.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "source_path": {
        ...
        ToolDefinition {
            ...
            description: "Clone a git repository and import all files. Much faster than fetching files individually via HTTP. Use this for importing entire repositories.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "url": {
                    ...
                    "destination": {
                        "type": "string",
                        "description": "Subdirectory in artifacts/imported/ to clone into (optional, defaults to repo name)"
                    },
                    "include_patterns": {
                        "type": "array",
                        "items": { "type": "string" },
        ...
        ToolDefinition {
            ...
            description: "Delete all CognOS browser data: cookies, logins, localStorage, cache. Closes any running browser first. Use when the user wants to start fresh.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {},
                "required": []
        ...
        ToolDefinition {
            ...
            parameters: json!({
                ...
                "properties": {
                    "id": {
                        "type": "string",
                        "description": "Know-how ID as shown in the know-how list (e.g., 'cognos/cross-workspace')"
                    }
                },
                "required": ["id"]
            }),
        ...
        ToolDefinition {
            ...
            description: "Connect an OAuth account (Google, Microsoft, GitHub, etc.) so CognOS can make authenticated API requests on the user's behalf. Opens the user's browser for authorization. Supported providers: google, microsoft, github (or any custom OAuth 2.0 provider with client credentials configured).".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "provider": {
        ...
        ToolDefinition {
            ...
            description: "Run Claude Code to edit source code. By default, runs against the CognOS source tree. Use the `repo` parameter to target a registered external repository instead. ONLY use this when the user explicitly asks to modify code (CognOS or an external repo). Never use this for workspace tasks like web scraping, file manipulation, data processing, or anything the native tools can handle — use browser_open, web_search, http_request, run_python, read_file, write_file, etc. instead. If Rust backend files are changed, CognOS shows the user a toast suggesting a restart — the user must manually trigger the rebuild and restart for backend changes to take effect (do NOT promise that changes will be live shortly). Frontend (TypeScript/CSS) changes are picked up automatically.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "prompt": {
                    ...
                    "repo": {
                        "type": "string",
                        "description": "Repository ID or name from manage_repositories. When provided, the Claude Code session starts in that repo's working directory instead of the CognOS source tree. Omit to target CognOS itself."
                    },
                    "allowed_tools": {
                        "type": "string",
