...
pub fn database_url() -> String {
    std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://cognos:cognos@localhost:5432/cognos".to_string())
}
...
pub use system_docs::{is_system_doc_path, SystemDocsStore};
...
pub fn commit_index(repo: &git2::Repository, message: &str) -> Result<String, git2::Error> {
    ...
    let sig = git2::Signature::now("CognOS", "cognos@local")?;
    ...
    match name {
        ...
        "http_request" => {
            ...
            if let Some(path) = args["temp_path"].as_str() {
                format!("Fetching {} → .cognos/tmp/{}...", url, path)
...
mod tests {
    ...
    fn migrate_top_level_prompts_to_triggers() {
        let dir = std::env::temp_dir().join("cognos_test_migrate_prompts");
    ...
    fn migrate_app_prompts_to_intents() {
        let dir = std::env::temp_dir().join("cognos_test_migrate_app_prompts");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn migrate_skips_if_already_done() {
        let dir = std::env::temp_dir().join("cognos_test_migrate_idempotent");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn migrate_empty_prompts_dir_removed() {
        let dir = std::env::temp_dir().join("cognos_test_migrate_empty_prompts");
        let _ = std::fs::remove_dir_all(&dir);
        let prompts_dir = dir.join("data/prompts");
        std::fs::create_dir_all(&prompts_dir).unwrap();
        std::fs::write(
