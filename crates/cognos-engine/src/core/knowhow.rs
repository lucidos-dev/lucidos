...
///
/// Engine-shipped reference docs live in `<repo>/system-docs/` and are
/// loaded separately via [`crate::core::SystemDocsStore`] — they cannot be
...
/// e.g., `knowhow/cognos/cross-workspace.md` → `cognos/cross-workspace`
...
impl KnowhowStore {
    ...
    /// Load full content for a specific know-how file.
    /// The id may contain forward slashes for files in subdirectories (e.g., "cognos/cross-workspace").
...
mod tests {
    ...
    fn load_summaries_discovers_files_in_subdirectories() {
        ...
        write_knowhow_file(
            &kh.join("cognos").join("nested.md"),
            "Nested",
            "Nested content.",
        );
        write_knowhow_file(
            &kh.join("cognos").join("deep").join("deep-file.md"),
            "Deep",
            "Deep content.",
        );
    ...
    fn load_merged_summaries_local_overrides_shared() {
        ...
        write_knowhow_file(
            &shared.join("cognos.md"),
            "CognOS (shared)",
            "Shared CognOS knowhow.",
        );
