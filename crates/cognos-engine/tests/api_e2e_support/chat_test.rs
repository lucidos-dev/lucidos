...
async fn parent_thread_id_propagates_to_projection() {
    ...
    assert_eq!(
        initiator, "system",
        "sender=system must mark the thread system-initiated"
    );
}

/// Sender=user keeps threads user-initiated (regression check for the default path).
#[tokio::test]
#[ignore]
async fn user_sender_means_user_initiated() {
    let client = http_client();
    let pool = sqlx::PgPool::connect(&db_url())
        .await
        .expect("Failed to connect to E2E workspace database");
    ...
    assert_eq!(
        parent, None,
        "user-sender requests have NULL parent_thread_id in projection"
    );
    assert_eq!(initiator, "user", "sender=user produces initiator=user");
}
...
/// `sender` is mandatory — requests omitting it must be rejected (not silently
/// defaulted to user) so the spawn-thread skill and other callers cannot
/// accidentally produce mislabeled threads.
#[tokio::test]
#[ignore]
async fn missing_sender_returns_400() {
    let client = http_client();
    let url = format!("{}/api/chat/stream", base_url());
    let body = serde_json::json!({
        "message": "test",
    ...
    assert!(
        ...
        "missing sender must be rejected as a bad request, got {status}",
    );
}
