    ...
    vec![
        ...
        ToolDefinition {
            ...
            description: format!(
                ...
                When the task finishes, returns the final tail with finished=true. That is guaranteed even if your drain lands at the exact moment the task completes: the engine keeps a completed task drainable for a few minutes, so a finish never costs you the result. \
                STOP polling once you see finished=true. Nothing new can arrive: a repeat call inside that few-minute window returns an EMPTY window (you already drained the output), and after it, calls fall back to the event store and return the FULL final stdout/stderr again, which wastes context. \
                ...
                THIS TOOL WAITS FOR OUTPUT, NOT FOR STATE. If what you are actually waiting on is a change in Lucidos (a thread going idle, a change being proposed, a coding agent finishing, a trigger firing, a domain event), do not drain-loop for it: call `await_event` instead. It parks the thread at zero cost, wakes on the exact event, and survives a restart, none of which a drain loop does. \
