...
///
/// Kept in step with the gateway's copy in
/// `crates/lucidos-gateway/src/net_config.rs` (the two modules are deliberately
/// duplicated, ADR 0014 section 1): both surfaces must detect the same address,
/// or the picker would offer one and per-workspace Settings would not.
...
/// "tailscale")` works in dev (rich shell `PATH`) and silently fails in a
/// packaged engine, which inherits the gateway's environment and therefore has
/// **no `PATH` at all**: the spawn errors, detection returns `None`, Settings
/// offers no address to click, and the Tailnet / IP mode becomes unselectable
/// because an empty address is not a valid bind. Same shape as
/// `runtime::claude_code::resolve_claude_binary`. The bare-name fallback keeps a
/// custom install working wherever a `PATH` does exist.
...
mod tests {
    ...
    fn tailscale_binary_prefers_the_first_existing_candidate() {
        // A packaged engine inherits the gateway's env, which has no PATH, so
        // detection MUST resolve an absolute path when one exists.
