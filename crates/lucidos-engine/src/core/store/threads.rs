...
impl EventStore {
    ...
    ///
    /// Also bypasses the per-source rank cap for threads that *need user action*:
    /// `cc_has_changes=TRUE` (Apply/Discard live in REVIEW), `status='waiting_for_user_answer'`
    /// (Question card lives in REVIEW), `status='failed'` (error indicator lives in REVIEW).
    /// Without the bypass, a CC thread with pending changes pushed past the per-source window
    /// vanishes from the drawer entirely — Diff/Apply/Discard become unreachable until the
    /// user manually scrolls far enough to trigger `get_older_threads`. Plain `section='unread'`
    /// rows that aren't actionable (e.g. an old idle chat the user never dismissed) stay
    /// subject to the cap so REVIEW doesn't drown in unresolved-but-uninteresting history.
    pub async fn get_recent_threads(
        ...
        let sql = format!(
            ...
            WHERE t.rn <= $2 OR t.cc_has_changes = TRUE OR t.status = ANY($3) \
    ...
    async fn get_recent_threads_always_includes_actionable_threads_beyond_window() {
        ...
        for i in 0..18 {
            ...
            let (status, cc_has_changes, section) = match i {
                15 => (ThreadStatus::Idle.as_str(), true, "unread"),
                16 => (ThreadStatus::WaitingForUserAnswer.as_str(), false, "unread"),
                17 => (ThreadStatus::Failed.as_str(), false, "unread"),
                _ => (ThreadStatus::Idle.as_str(), false, "default"),
            };
            sqlx::query(
                "INSERT INTO thread_summaries \
                 (thread_id, title, source, message_count, last_activity, has_response, \
                  status, cc_has_changes, section) \
