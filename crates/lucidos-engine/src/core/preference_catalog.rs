...
pub const INTERNAL_KEYS: &[(&str, &str)] = &[
    ...
    ("voice_input_device", "which microphone a call opens on one device: a browser's own opaque handle for it, which means nothing anywhere else. The user picks it by holding the call control, and you cannot know what the ids on their machine stand for"),
