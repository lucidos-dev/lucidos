...
pub(super) async fn install_post_commit_hook(worktree_path: &Path, thread_id: uuid::Uuid) {
    ...
    // Engine serves HTTPS when LUCIDOS_TLS_CERT/KEY are set (see main.rs's
    // axum_server::bind_rustls branch). The hook curl call must match the
    // engine's actual scheme — http:// against an HTTPS listener returns a
    // TLS Alert, curl exits 1, `|| true` swallows it, and ChangeProposed
    // never fires. Detect at install time and bake the scheme in.
    let tls_enabled = std::env::var("LUCIDOS_TLS_CERT").is_ok()
        && std::env::var("LUCIDOS_TLS_KEY").is_ok();
    ...
    let script = render_hook_script(thread_id, &api_port, tls_enabled);
    ...
    log!(
        "[ClaudeCode] commit-hook installed at {} (thread {}, port {}, tls {})",
        hook_path.display(),
        thread_id,
        api_port,
        tls_enabled
...
/// - Match the engine's actual scheme: `https://` + `-k` when the engine
///   is bound with TLS (the production default; localhost self-signed
///   cert), `http://` otherwise. A scheme mismatch silently breaks the
///   per-commit ChangeProposed path because the failure is swallowed by
///   `|| true`.
fn render_hook_script(thread_id: uuid::Uuid, api_port: &str, tls_enabled: bool) -> String {
    let scheme = if tls_enabled { "https" } else { "http" };
    // -k (--insecure) is required for the localhost self-signed cert the
    // engine generates by default. The risk is nil — same machine, same
    // user, and the engine still validates thread_id + sha on receipt.
    let insecure_flag = if tls_enabled { " -k " } else { " " };
    format!(
        "#!/bin/sh
...
[ -n \"$SHA\" ] || exit 0
(curl -fsS{insecure}-m 5 -X POST \"{scheme}://127.0.0.1:{port}/api/internal/commit-made\" \\
...
mod tests {
    ...
    fn rendered_script_contains_thread_id_and_port() {
        ...
        let script = render_hook_script(tid, "8765", false);
    ...
    fn rendered_script_starts_with_sh_shebang() {
        let script = render_hook_script(uuid::Uuid::nil(), "8080", false);
        assert!(script.starts_with("#!/bin/sh"));
    }

    #[test]
    fn rendered_script_uses_https_and_insecure_when_tls_enabled() {
        // Engine serves HTTPS whenever LUCIDOS_TLS_CERT/KEY are set (the
        // default for any real workspace). A hook hardcoded to http:// hits
        // the TLS layer, gets back a TLS Alert record, curl bails with
        // "Received HTTP/0.9 when not allowed", and `|| true` swallows it —
        // so ChangeProposed never fires and the diff card never appears.
        let tid = uuid::Uuid::nil();
        let tls_script = render_hook_script(tid, "5173", true);
        assert!(
            tls_script.contains("https://127.0.0.1:5173/api/internal/commit-made"),
            "TLS-enabled hook must use https:// scheme; got:\n{tls_script}"
        );
        assert!(
            tls_script.contains(" -k "),
            "TLS-enabled hook must pass -k to curl (localhost self-signed cert); got:\n{tls_script}"
        );

        let plain_script = render_hook_script(tid, "5173", false);
        assert!(
            plain_script.contains("http://127.0.0.1:5173/api/internal/commit-made"),
            "Plain-HTTP hook must use http:// scheme; got:\n{plain_script}"
        );
        assert!(
            !plain_script.contains(" -k "),
            "Plain-HTTP hook must not pass -k (no cert to ignore); got:\n{plain_script}"
        );
    }
    ...
    async fn end_to_end_commit_in_worktree_invokes_hook() {
        ...
        // The recorder listens for plain HTTP — clear any TLS env that the
        // parent process (e.g. the engine that spawned this CC session) may
        // have set, so the hook generates http:// rather than https:// + -k.
        std::env::remove_var("LUCIDOS_TLS_CERT");
        std::env::remove_var("LUCIDOS_TLS_KEY");
