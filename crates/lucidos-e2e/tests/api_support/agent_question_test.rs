...
/// question, the engine emits one to surface a "Thinking" spinner in the
/// timeline while CC processes the tool_result.
...
async fn answer_question_canceled_emits_answered_event() {
    ...
    assert_eq!(
        resume_markers, 1,
        "answer must emit a CodingAgentPromptSent so the timeline shows a Thinking spinner while CC resumes"
    );
}
