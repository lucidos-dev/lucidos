...
import { connectionStatus, restartRequired, updateAvailable, engineVersionReady, engineBuilding, engineBuildDetail, engineNewVersionReady, workspaceName, embeddingModelStatus, tailscaleServeRun } from '../../store/store';
 ...
 *    clickable itself; the click falls through to the brand label, which opens
 *    the workspace switcher where the Restart control lives.
...
export function controlPanelBadgeState(activityCount: number): BrandBadgeState {
  if (activityCount > 0) return 'busy';
  if (engineNewVersionReady() || updateAvailable.value) return 'ready';
  return 'none';
}

export function controlPanelBadgeTooltip(activities: BackgroundActivity[]): string | undefined {
 ...
 *  `controlPanelBadgeTooltip`, whose doc comment holds the rule. */
 ...
 *  spinning refresh icon while background activity runs, a `!` when a
...
export function BrandBadge() {
  ...
  const state = controlPanelBadgeState(activities.length);
  ...
  return (
    <button
      ...
      aria-label={tooltip}
      onClick={(e) => {
        // The badge sits INSIDE `.pane-header-brand-label`, whose onClick opens
        // the workspace switcher for any click on a child. Without this the tap
        // would pop that panel over the toast it just opened.
        e.stopPropagation();
...
export function ControlPanel({ layout }: { layout: 'desktop' | 'mobile' }) {
  ...
  // Delay-gate the skeleton: `switcherLoading` is true on the very first render
  // after the panel opens (the fetch effect has not run yet), so passing it raw
  // blinked three shimmer rows on every open against a fast local gateway.
  const showSwitcherSkeleton = useDelayedFlag(switcherLoading);
    ...
    <>
      ...
      >
        <div class="control-panel-workspace-list">
          <LoadingFade showSkeleton={showSwitcherSkeleton} skeleton={<WorkspaceSwitcherSkeleton />}>
          {gatewayWs.status === 'loaded' && (
            <>
