...
import { loadRepositories } from './actions/chat';
import { loadPreferences, flushPendingPreferenceWrites } from './actions/preferences';
import { loadPinnedApps } from './actions/pinnedApps';
import { loadReleaseNotices } from './actions/releaseNotices';
import { loadWebhookIngress } from './actions/webhookIngress';
import { loadWebhookRefusals } from './actions/webhookRefusals';
import { loadWorkspaceDisplayName } from './actions/workspace-label';
...
} from './actions/backgroundActivity';
import { isTauri } from '../utils/platform';
import { invoke } from '../utils/tauri';
import { refreshChangesState, restoreRestartState } from './actions/chat-changes';
import { restoreRepoSelectionFromStorage } from './actions/repositories';
...
import {
  ...
  hasDeepLinkParams,
} from './actions/notification-deeplink';
import { dispatchDeepLink } from './actions/in-app-notification-toast';
import { setupHashDeeplinkRouting } from './actions/hash-deeplink-router';
...
// point, return to the workspace picker. A cached shell that can't load would
// strand the user (the PWA auto-open-into-an-unreachable-engine case).
...
/** Start this document's client: the initial loads, the event stream, and every
 *  document-level listener and poll. Called by `main.tsx` before the shell chunk
 *  is evaluated, so the network work runs while the UI loads (ADR 0288).
...
export function startClient(): () => void {
  ...
  loadPreferences().then(() => {
    ...
    // dismissed. Re-derive now that preferences are known. The client-refresh
    // has no poll of its own. Without this a dismissed toast would flash on
    // cold start and linger until the next resume or PreferencesChanged. The
    // engine-switch also derives promptly instead of waiting out a 4s poll.
    void syncClientUpdateFromBuild();
    void checkEngineVersion();
  }).catch(() => { /* loadPreferences sets Loadable failed internally, and the UI shows the error */ });
  void loadPinnedApps();
  ...
  // every SSE frame so far has been missed. Without this read the status toast
  // would wait for the next frame, and a warm workspace would never learn the
  // model is ready.
  void loadEmbeddingModelStatus();
  // An Expose run narrates itself over a Tauri event and outlives the pane that
  // started it. So the listener belongs at startup beside the updater's, not
  // on the Mobile Access page.
  ...
  loadAllThreads().catch(() => {
    // Retry after 3s, which covers transient network failures on initial load.
  ...
  // suspended, the iOS PWA misses the hashchange of Safari's declarative-push
  // URL update. This router still routes that deep link on wake.
  ...
  loadRepositories();
  // Filter facets are the complete set of triggers/repos/apps with a thread.
  // The "Show" dropdown lists them all, not just those in the loaded window.
  ...
  // HMR restart would otherwise stack a duplicate on the same registration.
  ...
  if (!window.isSecureContext && !isTauri()) {
    ...
    // good). Take an insecure origin, such as plain http://<host> on a packaged
    // Linux install reached over the LAN. There the platform grants no service
    // worker and no push, and Chrome hides navigator.serviceWorker entirely. So
  ...
  if (isDevServerBundle()) {
    ...
    // module URLs and an unstamped sw.js. A worker here would cache the
    ...
    // A newly activated service worker raises the badge and the "New version
    // available" toast through the SINGLE reliable build-id check
    // (syncClientUpdateFromBuild), not a decision here. The check compares the
    // LOADED bundle against the served /sw.js. So it is right whether or not
    // this worker has claimed the page yet. It cannot disagree with the badge
    // either, since the same check drives that on startup and resume.
    // This same path also fires via controllerchange below, and both routes are
    ...
    navigator.serviceWorker.ready.then(reg => {
      if (stopped) return; // an HMR teardown ran before ready resolved
      updateFoundReg = reg;
      reg.addEventListener('updatefound', onUpdateFound);
    }).catch(() => { /* SW not ready in this environment; the update toast is best-effort */ });
  ...
  function onResume() {
    ...
    handleResume();
    // The OS could write the app-icon badge while we were away. iOS sets it
    // from the push payload's `app_badge`, and the SW sets it on Chrome. So
    // re-assert ours the moment we're back, before the reload. The icon then
    // agrees with the bell even if the engine is unreachable.
    syncWorkspaceAppBadge();
    // Reload the unread set on resume, so the bell badge reflects anything
    // that arrived while backgrounded. The load re-asserts the icon from the
    ...
    // just before backgrounding is applied here but missing on the server.
    // Resume is the first moment it can land. No-op when nothing is
    // parked. See store/actions/preferences.ts.
    void flushPendingPreferenceWrites();
    // Check for SW updates on resume. The iOS PWA never reloads, so this is the
    // only chance to detect new versions after the initial page load.
    navigator.serviceWorker?.getRegistration().then(reg => reg?.update()).catch(() => {});
    // Also re-check the BUILD_ID directly. A frontend build that landed while
    // we were away then lights the update badge. That holds even when iOS
    // leaves the browser's own SW update check slow or wedged.
    void syncClientUpdateFromBuild();
    ...
    // isn't replayed on reconnect), so re-poll authoritatively on resume. That
    // shows 'ready' if the build finished while away, never a stale spin.
    void checkEngineVersion();
    ...
    // belt-and-braces there. On a direct engine port it is the ONLY thing that
    // can correct the name. The switcher gates itself off there, with no
    // control plane on that origin. And the "next load" the label would wait
    // for never comes on iOS, which is why this handler exists at all. See
    // store/actions/workspace-label.ts.
    void loadWorkspaceDisplayName();
   ...
   *  burst went down a tunnel still reconnecting after the wake. And it
   *  grew with the workspace (85 thread-event GETs in one
  ...
  // and the Rust watchdog is the recovery path. A toast would be wrong on
  ...
  // (paneFocus.ts).
  const stopIframeFocusTracking = installContentPaneIframeFocusTracking();
