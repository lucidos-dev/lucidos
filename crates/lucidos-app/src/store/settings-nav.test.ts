...
describe('settings navigation', () => {
  it('keeps System last in the main settings list and owns Thread Queue, Backup, Memory, Disk Usage, Environment Variables, Network access, and Debugging as subpanels', () => {
    ...
    expect(keys).not.toContain('network-access');
    expect(keys).not.toContain('debugging');
    expect(SETTINGS_SYSTEM_SUBPANEL_ITEMS.map((item) => item.key)).toEqual(['thread-queue', 'backup', 'memory', 'disk-usage', 'environment-variables', 'network-access', 'debugging']);
    ...
    expect(settingsSubviewLabel('network-access')).toBe('Network access');
