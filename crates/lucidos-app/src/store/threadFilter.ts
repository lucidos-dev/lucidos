import { ALL_CHANNELS, type ThreadChannel } from './store';
 ...
 *  app-id only on `claude_code`. Under `claude_code`, repo and app selections
 *  are unioned — a thread passes if its repoId is selected OR (it's an app CC
 *  thread and) its derived app id is selected. Both selection sets empty
 *  means "all CC threads".
 *
...
export function threadPassesChannelFilter(
    ...
    if (ch === 'claude_code' && (repoSelection.size > 0 || appSelection.size > 0)) {
