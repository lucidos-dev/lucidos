...
- **Where:** `crates/lucidos-app/src/window_restore.rs`,
  `clamp_restored_geometry`; its caller in `app_window.rs` (`reopen_client`,
  after `size_main_window_for_its_workspace`).
- **Gap:** on that path the clamp reads `outer_position` and `inner_size`
  ...
  position on a display, unplug that display, and reopen from the tray: the
  clamp runs against the geometry `main` still had.
...
### An iOS home-screen app pairs separately from Safari
- **Where:** `crates/lucidos-gateway/src/auth.rs` (`COOKIE_DEVICE_CREDENTIAL`), `crates/lucidos-app/src/components/picker/PairingGate.tsx` (`pairingScreenBranch`), `system-knowhow/remote-access.md` § A phone installs before it pairs
- **Gap:** Pairing in an iOS Safari tab does not let the installed home-screen app in. Scanning the Access-page QR with the Camera app always opens Safari, never the app.
- **Why:** iOS gives a home-screen web app its own WebKit storage container. Cookies, localStorage, IndexedDB and the Cache API all live inside it, so nothing web-storage-shaped crosses from Safari. iOS also offers no way to route a scanned link into an installed web app, which is what Android's PWA scope capture does.
- **Status:** Worked around, not closed. Three paths carry a code across. The manifest's `start_url` pairs a fresh install. The shared pasteboard covers an install that already exists. The app's own camera reads the QR. The storage boundary itself is a WebKit platform rule with no engine-side fix.
- **The manifest channel needs the scan to stay on `/~/`.** Only the picker path is public, and only the gateway stamps the code from there into the manifest link an install reads. Two things protect that, and both were once wrong. `index.html`'s cold-start fast path stands down for a `pair` query instead of redirecting to the remembered workspace. And `serve_pairing_shell` stamps a code its caller arrived with instead of dropping it. See `docs/plans/2026-08-21-the-scanned-code-reaches-the-install.md`.
- **Residual:** the code in a `start_url` is fixed at install time. It still expires in five minutes, so a slower install opens with the code refused. A fresh code on the host cannot reach it. The app falls back to Scan or Paste, which is a recovery rather than a fix.
...
### `git_cmd_await_lock_file_does_not_retry_a_real_failure` flakes under host load
- **Where:** `crates/lucidos-engine/src/engine/git_ops_tests/merge.rs` (the ceiling assertion at the end of the test).
- **Gap:** The test proves a non-lock git failure returns on the first attempt rather than burning the retry budget. It does that with a wall clock, so under contention it intermittently fails by a small margin. Observed at 860.40ms against an 840.95ms ceiling on a full-suite run that took 166s, where the same run usually takes 100s. It passed immediately on a re-run of the module alone.
- **Why:** The ceiling is already calibrated per host, as `one_fork * 2 + LOCK_FILE_RETRIES * LOCK_FILE_BACKOFF / 2`, after a flat 500ms bound proved to be measuring fork latency. Calibration still times the control fork and the measured fork at two different moments, so load between them lands directly in the margin. The test's own comment concedes the limit: fork jitter on a loaded host is larger than one backoff, so no wall-clock bound separates one stray retry from none.
- **Status:** Open, same class as the `bash_background` row above. Re-run the module alone before treating a failure here as a regression; a real one fails deterministically. Closing it means asserting the retry COUNT rather than elapsed time, which needs a seam in `git_cmd_await_lock_file` that does not exist today.
...
### A dead MCP server keeps reading as Running until someone stops it
- **Where:** `crates/lucidos-engine/src/mcp/mod.rs` (the `running` registry), `crates/lucidos-engine/src/mcp/client.rs` (`request`)
- **Gap:** Nothing removes a server from the registry when its process dies. Settings keeps showing it as "Running, this session", with its tools priced into every request. `get_tool_definitions` keeps offering them until the user presses Stop or the engine restarts. Every call meanwhile fails with a clear error naming the server.
- **Why:** Death is only observed at call time, and not every call failure is death: a 30s timeout is not, nor is a tool-level error. Distinguishing them means a liveness concept the client does not have today. The frame cap added one case the engine causes deliberately, killing a server that breaks JSON-RPC framing. A server crashing on its own has always behaved this way.
- **Status:** Open. Closing it means marking the client dead on the failures that ARE terminal: a killed child, a closed stdout, a broken stdin pipe. `call_tool` would then evict such an entry, which also buys self-healing, since the next call restarts the server on demand.
...
### The command guard does not see through every shell-wrapper form
...
- **Gap:** **A wrapper behind a benign prefix.** `unwrap_shell_command` resolved the FIRST token, so `sudo bash -c '...'` and `env bash -c '...'` were not unwrapped and read as head `sudo`. The decorated forms (`\bash`, `"bash"`) are unwrapped, since 2026-08-06.
- **Why:** `unwrap_shell_command` is the one place the tail-discard in `tail_runs_more_commands` is sound. That soundness argument is written for a shell whose operands after the script set `$0`. Walking past a prefix means re-deriving it for whatever the prefix does with its own arguments.
- **Status:** **Closed.** `shell_token_index` now walks the whole preamble, so a shell behind a benign prefix OR an exec wrapper is unwrapped: `sudo bash -c`, `nohup bash -c`, `timeout 5 sh -c`, `flock <lock> bash -c`. The tail-discard is unaffected, since it still runs on the shell's own operands and the walk only decides WHICH token is the shell. Pinned by `a_wrapper_in_front_of_a_shell_still_unwraps_the_payload`.

  Two things it deliberately does not do. Unwrapping DISCARDS the preamble, so a code-injecting `VAR=` in it (`ENV=/tmp/rc bash -c 'echo hi'`) declines the unwrap rather than hand the fast path a bare inner command. A wrapper flag taking a SEPARATE value (`xargs -I {} sh -c`) also still hides the shell: without per-flag arity, `-0 grep` and `-I {}` are one shape.

  The other two forms closed on 2026-08-24, per `docs/plans/2026-08-24-command-guard-bypass-cluster.md`:
  - **Command substitution** is now classified. `substitution_bodies` extracts the inside of `$(...)`, backticks, `<(...)` and `>(...)`. `catastrophic_reason` and `bash_destruction_scope` recurse into each body behind a depth cap, so `echo $(rm -rf /)` hard-blocks instead of reading as head `echo`. An opener inside a single-quoted run is skipped, so a grep for the literal text is not a run. An unterminated opener is classified on its remainder.
  - **The raw head** is now the deliberate MATCHING rule rather than an accident. `segment_heads` (basenamed) derives what a grant stores; `segment_heads_as_written` (verbatim) is what a grant is matched against, so `Bash(ls:*)` covers `ls` and never `data/bin/ls`. The over-blocking worry that kept this open was about the two sharing one function, which they no longer do.

### An unattended coding-agent session auto-allowed anything the guard could not settle
- **Where:** `crates/lucidos-engine/src/engine/cc_permission.rs` (`classify_coding_agent_request`, `decide_unattended`), `docs/adr/0002-lucidos-agent-command-safety.md` § Addendum (2026-08-24)
- **Gap:** The unattended lane mapped `fallback_classify`'s `Safe` to `RequestVerdict::Benign`. So a shape the Safe fast path deliberately refuses was re-settled as an allow, with no card and no human: substitution, an `LD_PRELOAD=`/`PATH=` preamble, a path-qualified head, an out-of-workspace write, an executable git config. A `Bash` / `command_execution` request whose `command` could not be read fell to the same default. A stored session grant covered the same shapes, and on that lane it is checked BEFORE classification.
- **Why:** The lane was written to answer the three verdicts the classifier produced, and "the static pass declined" was not one of them: it arrived as the absence of a verdict.
- **Status:** **Closed (2026-08-24)** by `docs/plans/2026-08-24-command-guard-bypass-cluster.md`. A refusal is now `RequestVerdict::Unclassified`: it denies whatever the grant holds, and no stored grant covers one. Every unattended deny records the ordinary request/resolved pair, so the run is diagnosable. A merely UNRECOGNISED head (`cargo build`) still runs, which is the line the ADR addendum draws and defends. **One residual, accepted rather than overlooked:** the refusal set is coarser than an attack shape, so a `/tmp` log redirect, a `./scripts/x.sh` head and `sort /etc/passwd` are denied unattended too. Separating them needs per-head flag arity; a deny costs one request, not the run.

### The chat and trigger lane still settles a fast-path-REFUSED shape as Safe
- **Where:** `crates/lucidos-engine/src/engine/command_guard.rs` (`fallback_classify`), against the entry above, which fixed the same predicate on the coding-agent lane
- **Gap:** `JudgeInput.fast_path_refused` says the Safe fast path actively REFUSED this text, rather than merely missing its head: a command substitution, a code-injecting `VAR=` preamble, a path-qualified head, or a write outside the workspace. `cc_permission` reads that flag and denies. `fallback_classify` ignores it, so with the judge off or erroring, `bash -c "$(cat data/payload.sh)"` runs with no card on chat and auto-allows on a trigger. One predicate, two answers.
- **Why:** Not an oversight, and not cheap to close. `static_classify` makes EVERY python decline a Refusal, so returning `IrreversibleDanger` for the flag deletes the `ReversibleDanger` checkpoint lane for `run_python` outright and leaves `python_destruction_scope` dead. On bash it moves `echo "$(date +%F)" >> data/artifacts/report.md`, `./scripts/collect.sh` and `cmd > /tmp/out` from Safe to a permission card. It fails any trigger granted less than `Other`, including one granted exactly what the command does, since the derived category goes with it. That was written and reverted, unreviewed, on 2026-09-08.
- **Status:** Open, and it needs a decision rather than a patch: which REFUSAL REASONS deserve which lane. A substitution body can be anything, so distrusting its derived category is right. A `>/tmp` redirect or a path-qualified head is a KNOWN head with a path problem, and treating those alike is what produces the blast radius. The e2e test `a_command_destroying_only_ignored_content_leaves_no_undo_card` runs this path with the judge off and is the tripwire for any attempt.
...
### Three tool sources sit outside the gated registry tables
- **Where:** `crates/lucidos-engine/src/llm/tools/mod.rs` (`FAMILIES`, `CHAT_TAIL`), against the splice in `crates/lucidos-engine/src/engine/chat/process/run.rs`; `docs/adr/0088-tool-schemas-are-gated-by-capability-not-trimmed.md` Consequences.
- **Gap:** ADR 0088 rules that a new tool family must state its gate, or state that it has none. The *type* enforces it for the two registry tables: a row cannot be written without a `Gate`. Three sources are spliced outside them and can still arrive resident by omission. `send_notification` is a bare `push` between the two tables. `capability_manifest::llm_tools()` contributes 14 grouped schemas with no gate concept at all. MCP-discovered tools come from running servers.
- **Why:** The wire ORDER is the first cache segment's prefix, and reordering it rewrites the tools tier for every workspace. `send_notification` sits between the two tables and the manifest set sits after it, so folding either into a table would move schemas. The manifest is also a different owner (`crate::capability_manifest`), whose declared-parity model has no gate field; adding one is its own change. MCP tools are a workspace's own configuration already, so they are gated in the only sense that applies.
- **Status:** Open, and narrowed rather than closed. Every hand-written flat family is in a table, pinned by `every_offered_schema_comes_from_a_gated_registry_row`. So the omission cannot happen where new families are actually added. Closing the rest means giving `capability_manifest` a gate field, which is worth doing when a grouped domain first turns out to be unusable somewhere.

### The always-loaded meter bills two of the three turn-tail blocks
- **Where:** `always_loaded_areas` in `crates/lucidos-engine/src/engine/chat/process/system_prompt.rs`, against `chat::process::turn_clock::current_time_block`.
- **Gap:** The meter behind `always_loaded_context_stays_under_budget` bills the `[ENGINE BUILD]` and `[CLIENT URL]` blocks, both halves each, but not `[CURRENT TIME]`. So roughly 60 characters of engine-authored labels and scaffolding ship on every turn outside the only meter that watches always-loaded text.
- **Why:** The clock moved to the tail under ADR 0084 and was never added to the meter; the other two were added when they moved. Adding the clock now would move the ratchet for an unrelated reason. That attribution is exactly what the ceiling's doc comment exists to keep honest.
- **Status:** Open, and small. Fold it in on the next deliberate move of `ALWAYS_LOADED_BUDGET_CHARS`, billing the labels rather than the reading. The timezone name and the timestamp are workspace and per-turn values, not engine-authored text.
