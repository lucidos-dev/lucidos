...
- **Gap:** **A wrapper behind a benign prefix.** `unwrap_shell_command` resolves the FIRST token, so `sudo bash -c '...'` and `env bash -c '...'` are not unwrapped and read as head `sudo`. The decorated forms (`\bash`, `"bash"`) are unwrapped, since 2026-08-06.
...
- **Status:** Open, and narrowed to this one form. The other two closed on 2026-08-24, per `docs/plans/2026-08-24-command-guard-bypass-cluster.md`:
