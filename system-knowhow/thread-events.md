...
the end of each turn. So a thread idle apart from its running children reads
*Changes to review*, and the user can Apply or Discard its change. If you
commit again after its completion wakes you, that work comes back as a new
change of its own. An apply clears your branch's plan and harden markers, so
a Lucidos-source thread plans and hardens that new round again. Delegating work to a child is therefore
not a reason to hand your own change to a "carrier" child.
...
**A reply also says how long it took to say.** Speech covers a stretch of time,
while a step beside it is an instant. So the transcript reads a reply where the
talker BEGAN it rather than where the words stopped (ADR 0206). Without that, a
reply said over the first steps of a turn reads under the work it promised.
...
agent's own round trip and is the default; the rest are *auxiliary model calls*
and carry `producer: "auxiliary"`.
...
The last three run on the agent's own chat model. They own no preference, so
Settings offers no row for them. `judge_tool` owns none either, for a different
reason: it is TypeSafe or nothing.
...
The marker outlives the engine, which is the point. It is one of the three the *stranded queued message* predicate reads. So a retraction made while the engine was down still beats the recovery on the next resume (ADR 0236).
