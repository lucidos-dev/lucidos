...
**A second gate applies to every carrier: the recursion cap.** An event a trigger's own run emits dispatches at that run's chain depth, one deeper than the event that fired it. Past `MAX_EVENT_TRIGGER_DEPTH` (3, in `crates/lucidos-engine/src/scheduler/task_runner.rs`) the event is still stored and still reaches SSE, it just fires no further triggers. So a trigger subscribed to an event its own run emits gets three fires and then stops, instead of looping forever.

The cap never touches an ordinary turn, whose events dispatch at depth 0. It is a backstop, not permission: the hard rule against subscribing to your own run's events lives in `system-knowhow/triggers.md`. Three paid model calls per mistake is still three too many.
