...
| Event | When it fires | Volume | Persisted | Triggerable |
|---|---|---|---|---|
| `VoiceSessionStarted` | A voice session opened on this thread. Carries `session_id: Uuid`, and the `actor` (from `EventMeta`) is the device that opened the socket. Exactly one session may be live per thread, so a second upgrade is refused and writes no second row. | lifecycle (rare per thread) | yes | yes |
