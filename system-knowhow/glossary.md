...
- **`waiting`** when the thread still holds a live *event wait*. It stopped on purpose and something will wake it, the same fact its own Waiting status dot carries. Waiting rows keep full-strength text, a clock marker and a `waiting` tag.
...
### What's New
The release notes panel, at **Settings > System > What's New**. It lists every published Lucidos release newest first, with the one you are running open and marked **Running**, and the rest as rows you expand. The notes are the same prose as the project's changelog, and they travel **inside the engine binary**, so the panel works offline and on an installed copy with no source checkout: it is the history of what you HAVE.

A release the updater is **offering** is the exception, and it sits above that list marked **Available**. It has to come from somewhere else, because a version being offered to you is by definition newer than the copy of Lucidos showing it, so its notes are not in that binary's history; they arrive with the update check instead. That is why the offer's notes appear only where there is a real update pending, and why nothing falls back to the installed list to fill the gap: doing so would show you what your CURRENT version contains under the heading of the one you were about to install.

Three ways in, all of them places the question comes up. The Lucidos menu's version row (tap the Lucidos mark in the header) opens it, and carries a dot when this device has not read the notes for the release it is running. Settings > System's update notice links to it. So does the update toast, next to Update & restart. The dot is per device, so the same workspace tells you once on your laptop and once on your phone.
