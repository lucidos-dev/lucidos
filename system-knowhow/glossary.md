...
`waiting` is not the end of the road: a wait that resolves without the agent picking the list back up settles those items to `abandoned` at the next terminator, so a parked list cannot read as parked forever. `abandoned` is final in the other direction, since subscribing to something later does not un-abandon an item. To avoid the settle entirely the agent must either finish the list (every item `completed`) or call `todo_write` with `[]` to drop it explicitly.
