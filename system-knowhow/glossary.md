...
### Knowhow
How to achieve an *intent*, in technical terms: API details, data formats, quirks, workarounds, fallbacks. Evolves every time Lucidos learns something new. Lives in `data/knowhow/<id>.md` (shared) or scoped to an app / trigger. Discovered at runtime by the LLM via the `load_knowhow` tool, matched by the file's frontmatter `name` + `description`. Every knowhow file is either a *knowhow doc*, listed for routing, or a *knowhow reference*.
...
Whether a *plugin*'s shipped content has been locally edited since it was installed. The user, the *Lucidos Agent*, or a *coding-agent thread* changed an app, knowhow, script, or trigger the plugin owns. Surfaced as a **Modified** badge on the plugin's row in the *Plugins panel*, whose tooltip lists the changed paths. Updating merges those edits rather than discarding them (see *plugin local patch*), and the install panel states the outcome per file. **Derived, not stored**: the engine diffs current `data/` content against the install commit, so it self-heals on a revert and stays true after a merge. An added file inside a plugin's app directory counts; a brand-new file in a shared root belongs to no plugin.
