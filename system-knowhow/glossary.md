...
### Current section
The thread-drawer section holding the live working set: every *thread* that isn't *saved* or archived — whether running (the system's turn, shown via the *Active* row indicator), awaiting the user (their turn), or recently idle. Replaces the former Active + Review sections, merged so a thread no longer jumps sections every turn. Current is ordered by *thread* creation time (newest first) — a stable order that doesn't reshuffle as agents work or a thread gains a call-to-action. Attention-needing threads (awaiting answer/permission, pending *change*, failed) are NOT bubbled to the top; they're surfaced by a count badge and the attention filter icon instead. That count, summed with the *Saved* section's, drives the thread-drawer toggle badge. The drawer's other sections are *Saved* and Archive.
