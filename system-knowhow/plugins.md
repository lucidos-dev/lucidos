...
The marketplace list itself is read live from the registry on every request. So a rename shows up with no scan behind it, and a marketplace the user removed can never contribute cached plugins. A scan announces both ends, as **transient** events (SSE only, never stored): `PluginCatalogScanStarted` when it begins and `PluginCatalogScanned` when it lands, the latter carrying `failed`. They raise and lower the panel's "Updating…" cue on every connected client, including for the scheduler's own pass that nobody asked for.
