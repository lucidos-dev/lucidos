...
| `CodingAgentInputRead` | The coding agent took in an input the engine forwarded. Claude Code reports it by replaying the input; Codex when its turn for the input starts. Carries `input_event_id`, the event that carried the input (the `MessageReceived` for a message). Until it lands the input is owed, so the session keeps its subprocess at idle (ADR 0268). The transcript marks the message "Sent", then "Read". | One per forwarded input |
...
Two agent-driven shapes no longer supersede a question the user can still answer. A child-completion wake or an event-wait delivery waits in the agent's input queue (ADR 0255). An agent-sent message, such as a parent's follow-up, becomes a *held message* (ADR 0256). The tool result tells the agent its question was replaced. It also says the replacement arrives as the agent's next input.
...
coding-agent child parked on a card becomes a *held message* instead: the card
stays pending, and the child receives the message once a human answers the card
...
A tool call that was *running* at teardown gets no such reason: Claude Code closes it in its own transcript as "The user doesn't want to proceed with this tool use" plus `[Request interrupted by user for tool use]`. The resumed session replays that text. So every resumed turn whose gap ends in a restart abort carries a restart line in its turn-gap note (see § "The turn-gap note" below). The line says the shutdown wrote those lines, the user refused nothing, and the call is safe to run again. Without it, the resumed agent reads the interruption as a rejection and abandons its plan.
...
**A non-empty block closes by disclaiming the topic.** The notes quote commit subjects, branch names and file paths. They land right before a message that often leans on a pronoun, and those nouns win it. After a Diff-button change applied, a request for more height bound to the note's "composer's Diff" and grew the composer, not the glyph. So the block's last line calls those lines engine status, not the subject. A turn with no note gains nothing.
...
**The restart row reads only the newest coding-agent terminal in the gap.** Claude Code writes that rejection text itself when the engine stops it at teardown, and the text cannot be changed. So the resume has to name the real cause, or the agent treats a restart as the user saying no and stops retrying the work. The row stays out once a later coding-agent terminal follows the abort, because a resumed turn has already run past the restart. Other abort causes, user cancels and chat-channel aborts never produce it. It rides every resume path: the automatic resume after a switch, a Continue click, and the user's next message after a crash.

**That row skips what an event wait delivered.** A coding agent's `lucidos background-task run` arms an *event wait* on the completion, and the delivery is the prompt the re-opened turn carries. So the note drops a completion an `EventWaitDelivered` names. What reaches the note is the one nobody delivered, such as a task whose wait a subscription cap refused. See § `BackgroundBashCompleted` in `system-knowhow/thread-events.md`.
...
Adding `is_external_repo: { $ne: true }` scopes to the engine repo; `requires_restart: true` scopes to changes that need a new engine version, and so on. The same condition language works across all per-payload fields.
