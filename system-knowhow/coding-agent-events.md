...
**A turn's verdict is sticky.** There are two: `failed` (the turn errored) and `paused` (an engine restart interrupted it, see the next paragraph). Only a real *start* event clears one (`MessageReceived`, `CodingAgentUserMessageSent`, `UserPromptInjected`, `CodingAgentPromptSent`, `ContinuationRequested`), because only those mean new work was actually requested. Every event that merely *closes out* the ended turn routes its status write through the shared `preserving_verdict` guard (`engine/event_bus/mod.rs`) and leaves the verdict alone: the trailing activity stream, `CodingAgentIdled`, `SessionEnded`, and `ResponseCanceled`. So the status dot persists until the user sends a follow-up or clicks Continue.

**An interruption is `paused`, not `failed`.** `AbortCause::status_sql()` splits on `AbortCause::is_transient()`. The two transient causes settle the thread at `paused`, because nothing failed and the turn is expected to come back: `EngineShutdown` (the *Switch to new version* teardown) and `RecoveryAfterRestart` (the boot sweep's crash boundary). `StaleSettle` keeps the cancel-style idle mapping, and every remaining cause keeps the red `failed`: `SafetyNet`, `ProcessKilled`, `SessionDropped`. Pending changes still override all of them to `waiting`. Before this split, a version switch painted every in-flight thread with the red error dot for work the engine was about to resume by itself.
