...
**`failed` is the turn's terminal verdict, and it is sticky.** Only a real *start* event — `MessageReceived`, `CodingAgentUserMessageSent`, `UserPromptInjected`, `CodingAgentPromptSent`, `ContinuationRequested` — clears it, because only those mean new work was actually requested. Every event that merely *closes out* the failed turn routes its status write through the shared `preserving_failed` guard (`engine/event_bus/mod.rs`) and leaves the verdict alone: the trailing activity stream, `CodingAgentIdled`, `SessionEnded`, and `ResponseCanceled`. So the red error dot persists until the user sends a follow-up or clicks Continue.
