...
| `CodingAgentPermissionRequest` | The coding agent asked to confirm a tool call. Claude Code raises this through its MCP permission-prompt subprocess (Edit/Write/Bash on a path outside cwd, anything under `.claude/` or `.git/`); Codex raises it through the app-server approval bridge for sandbox-escaping commands and out-of-worktree file changes. The user resolves it via `POST /api/v1/permission/<request_id>/{allow,deny}`. | Per tool call needing consent |
