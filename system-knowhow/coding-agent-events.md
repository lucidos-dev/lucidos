...
| `CodingAgentThoughtStreamed` | Each chunk of streamed reasoning/thinking the coding agent produces before its visible output. CC sends it as a `stream_event` `thinking_delta` (text on `delta.thinking`; the persisted CC JSONL keeps only an encrypted signature, so the live stream is the only source); Codex sends `item/reasoning/summaryTextDelta` / `textDelta` (app-server) or a `reasoning` item (exec). Coalesced into a few rows per turn and rendered as the live "Thinking" step's content so a long reasoning pass shows progress. **Live on Codex, dormant on CC.** Codex: both drivers set `model_reasoning_summary=detailed` (codex's default summary mode emits no reasoning notifications at all, verified live on codex-cli 0.142.5), so Codex threads stream reasoning *summaries* into this event. CC: **dormant for the current models, and NOT provider-specific:** Anthropic's `thinking.display` defaults to `omitted` on every current model (Fable 5.1 and 5, Opus 5, Opus 4.8 and 4.7, Sonnet 5), so the `thinking_delta` carries empty text (signature only) and this event does not fire. That holds on **both** Vertex and the first-party Anthropic API (empirically confirmed), and even with `--thinking-display summarized` forced (an upstream Claude Code limitation in its headless `stream-json` path; the raw chain of thought is never returned regardless, since a summary is the most any display mode yields). Switching CC's provider does **not** fix it. See `docs/temporary-measures.md` § `cc-reasoning-dormant`. | **no (blocked: per-token streaming)** |
...
**That row filters itself, and nothing else has to.** A completion's watcher normally pushes a resume prompt at the parked session, and that push emits `CodingAgentPromptSent`, which is a turn BOUNDARY. So a completion the agent already heard about falls outside the next gap by construction. What reaches the note is the one nobody delivered: the push is suppressed during a shutdown, and a crash takes the watcher with it. See § `BackgroundBashCompleted` in `system-knowhow/thread-events.md`.
