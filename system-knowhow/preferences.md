...
| `model_voice_transcriber` | global | a model id | `gpt-4o-mini-transcribe` | Model turning the caller's speech into text inside a *voice session*. The second and last model in the voice loop: nothing translates and nothing summarises, and the agent's own answer reaches the talker as written. Which language it is pinned to comes from `language`, not from here. |
