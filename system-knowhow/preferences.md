...
|---|---|---|---|---|
| `language` | global | text | (detected from conversation) | Language for responses + session summaries (e.g. "English", "Norwegian"). A *voice session* speaks it, and pins its transcriber to the matching ISO-639-1 code. |
...
| `model_voice_transcriber` | global | a model id | `gpt-4o-mini-transcribe` | Model turning the caller's speech into text inside a *voice session*. The second and last model in the voice loop: nothing translates and nothing summarises, and the agent's own answer reaches the talker as written. Which language it is pinned to comes from `language`, not from here. Settings offers five ids, and `gpt-live-transcribe` is the one built for a live microphone: it streams the transcript as the caller speaks. The engine sends its language pin as `languages`, an array, where every other model reads the singular `language`. `gpt-transcribe` transcribes a turn once it is committed and can report the language it detected, and the older three are `gpt-4o-mini-transcribe`, `gpt-4o-transcribe` and `whisper-1`. |
...
- `voice_input_device`: which microphone a call opens, per device. The value is
  a browser's own opaque handle for one, so it means nothing in another browser
  and nothing on another machine. The user picks it by holding the call control
  in the composer. You cannot know what the ids on their machine stand for, so
  never write one.
