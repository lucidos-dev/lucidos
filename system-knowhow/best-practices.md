...
10. **Where the file tools can reach.** Three prefixes resolve, and everything else is rewritten or refused:
    ...
    Everything else under `.lucidos/` is engine runtime state (`worktrees/`, `exhaust/`, `engine.pid`) and is refused in both directions. `..` and absolute paths are rejected outright. To read engine source, or anything else outside the workspace, use `run_bash` (`sed -n`, `cat`). The Lucidos repo is at `~/projects/lucidos` and is NOT under `data/artifacts/`.
