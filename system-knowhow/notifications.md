...
**Tap routing — same as the web-push tap.** Taps arrive through the delegate's `userNotificationCenter:didReceiveNotificationResponse:` callback (on the main thread). The delegate keys off the response's notification `identifier` (set to the engine `notification_id` at `show` time) to look up the stashed deep link from an in-process map, focuses the main window (the OS also activates the app), and emits a `native-notification-tapped` Tauri event carrying the SW-message-shaped deep link. A dismiss (`UNNotificationDismissActionIdentifier`) drops the stashed link without routing. The page (`setupNativePushTapRouting`, wired once in `useStartup`) listens, runs it through `parseDeepLinkFromSwMessage` → `dispatchDeepLink` — the **exact** router the web-push service-worker tap uses — so a native tap marks-read and navigates identically to a web-push tap (modal opens the inbox, navigate deep-links the thread/app, none just marks read). The deep-link map is in-process only: a tap after the app fully quit can't route (matches the prior behaviour — the bell badge driven by `NotificationCreated` is the durable signal), and it is soft-capped so un-acted notifications can't grow it unbounded.
...
The native-desktop OS surface is exercised in `actions/native-push.test.ts` (driven by the `NativePushRequested` SSE) — banner on Tauri + fresh + non-active (with the deep link forwarded in SW-message shape), the no-op gates (not Tauri, stale frame, page active), the empty-title `Lucidos` fallback, and `setupNativePushTapRouting` routing a `native-notification-tapped` event through `dispatchDeepLink`.
