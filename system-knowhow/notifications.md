...
2. **OS surface** — an OS-level notification banner outside the Lucidos UI. **Two transports, picked by client:**
   ...
   - **Native desktop** (Tauri app) — a native macOS notification driven by the `NativePushRequested` SSE (§4), rendered + tap-routed by the app's `show_native_notification` command (`mac-notification-sys`). The embedded WKWebView can't subscribe to Web Push, so the engine reaches the desktop app over its already-open SSE stream instead of a push service.
...
- **Why a custom command, not `tauri-plugin-notification`.** The plugin's desktop `show()` is fire-and-forget — `notify-rust` displays the banner and the click is never reported (its `onAction` is mobile-only) — so it can't drive tap routing. Instead the command drives `mac-notification-sys` (the crate notify-rust uses under the hood) directly, with `wait_for_click(true)`, on a worker thread (the call blocks until the user interacts). See `show_native_notification` in `crates/lucidos-app/src/lib.rs`.
...
**Enabling.** There is no web-push subscription to create on desktop, and the macOS notification path (`mac-notification-sys` / the deprecated `NSUserNotificationCenter`) has no JS-queryable permission — macOS shows banners per the user's System Settings and prompts on first delivery. So "enabling push" on the Tauri app (the same `initPushSubscription` entry point the LLM `enable_push_notifications` tool and the settings toggle drive) is a no-op success; `handleNativePushRequested` renders banners with no further setup. In dev the binary isn't a packaged `.app`, so the command borrows Terminal's bundle identifier (mirroring notify-rust) to deliver at all; a packaged build uses `com.lucidos.app`.

**Tap routing — same as the web-push tap.** On a click `mac-notification-sys` returns `NotificationResponse::Click`; the command focuses the main window (the OS also activates the app) and emits a `native-notification-tapped` Tauri event carrying the SW-message-shaped deep link. The page (`setupNativePushTapRouting`, wired once in `useStartup`) listens, runs it through `parseDeepLinkFromSwMessage` → `dispatchDeepLink` — the **exact** router the web-push service-worker tap uses — so a native tap marks-read and navigates identically to a web-push tap (modal opens the inbox, navigate deep-links the thread/app, none just marks read). Caveat: the worker thread blocks in `wait_for_click` until the user clicks or dismisses the banner, so a handful of un-acted native notifications hold a thread each — fine at Lucidos's low notification volume.
