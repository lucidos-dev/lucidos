...
| `system-knowhow/lucidos-cli.md` | What scripts and CC subprocesses use for `data.*` writes, `events.*` emits, and `proxy` calls to external APIs (preferred over raw `curl -H "Authorization: ..."` with `$CRED_*`) |
| `system-knowhow/cross-workspace-threads.md` | `lucidos send-thread` shape (only if a script/trigger crosses workspaces) |
...
Per the engine prompt's taxonomy section and the worked example in `docs/taxonomy.md` (mirrored in best-practices). For each `TriggerCreated`:
...
- Each ID in `run.knowhow` resolves to an existing file.
