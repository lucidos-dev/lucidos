...
if [ ! -f "$PGDATA/PG_VERSION" ]; then
    ...
    sudo -u postgres /usr/lib/postgresql/16/bin/initdb -D "$PGDATA" --auth-local=trust --auth-host=trust
...
sudo -u postgres /usr/lib/postgresql/16/bin/pg_ctl -D "$PGDATA" -l "$PGDATA/postgresql.log" start

# Wait for PostgreSQL to be ready
echo "Waiting for PostgreSQL..."
for i in {1..30}; do
    if sudo -u postgres /usr/lib/postgresql/16/bin/pg_isready -q; then
        break
    fi
    sleep 1
done
