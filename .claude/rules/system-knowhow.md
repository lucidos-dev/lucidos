...
paths:
  ...
  # The other two surfaces § Settings -> Access documents in detail. Both
  # arrived after the entry above and matched nothing, so editing either
  # loaded no rule at all.
  - "crates/lucidos-app/src/components/settings/AddDeviceSection.tsx"
  - "crates/lucidos-app/src/components/picker/PairingGate.tsx"
  # The device LIST, which remote-access.md documents and which moved off
  # MobileAccessPage.tsx when the two lists merged. Same trap as the two above:
  # while it rendered from a listed file the rule loaded by accident.
  - "crates/lucidos-app/src/components/settings/deviceList.ts"
  - "crates/lucidos-app/src/components/settings/pairedDevices.ts"
...
| `crates/lucidos-engine/src/scheduler/push.rs`, `crates/lucidos-engine/src/core/device_presence.rs`, `crates/lucidos-engine/src/api/presence.rs`, `crates/lucidos-engine/src/api/presence_pong.rs`, `crates/lucidos-app/public/sw.js`, `crates/lucidos-app/src/utils/pageActive.ts`, `crates/lucidos-app/src/store/actions/device-presence.ts`, `crates/lucidos-app/src/store/actions/presence-pong.ts`, `crates/lucidos-app/src/store/actions/in-app-notification-toast.ts`, `crates/lucidos-app/src/store/actions/notification-visit.ts`, `crates/lucidos-app/src/store/actions/visitKeys.ts`, `crates/lucidos-app/src/store/actions/push.ts` (notification fan-out, presence, PresenceCheck, the §4 matrix, the *seen target* rule, service worker push, push subscriptions) OR the **native desktop** half: `crates/lucidos-app/src/notifications.rs`, `crates/lucidos-app/src/store/actions/native-push.ts`, `crates/lucidos-app/src/utils/nativeWindow.ts`, and `route_native_tap` / `front_window` / `emit_window_active` in `crates/lucidos-app/src/app_window.rs` (banner show + dismiss, the UN delegate, tap window-targeting, the pending-tap stash, the native-active bridge) | `system-knowhow/notifications.md` § matching `§N` section (§2 for the fan-out matrix, §3 for the PresenceCheck protocol, §4 for the in-app surface AND its "Native desktop OS surface (Tauri)" subsection), AND `system-knowhow/glossary.md` if the change shifts what "active device" / "in-app surface" / "OS surface" / "source event" mean |
| `crates/lucidos-engine/src/net_config.rs` + `crates/lucidos-gateway/src/net_config.rs` (the *network bind* resolution order, `network.toml` shape) or `crates/lucidos-tailscale/**` (tailnet detection, the MagicDNS lookup, the CLI candidate list) or `crates/lucidos-app/src/mobile.rs` + `components/settings/MobileAccessPage.tsx` (the Mobile Access page: its `tailscale up` / `tailscale serve` commands, the connect URLs, the four Tailscale states, and which half is desktop-only) | `system-knowhow/remote-access.md` (§ Settings → Access for the page's own behavior, § Network bind, § Route B: the exact `serve` invocation and where the packaged app runs it), AND `README.md` § Remote access if the install-time `--bind` / `--tls-cert` contract changed |
...
- A change to the plugin manifest schema (required keys, validation rules, install / uninstall flow) must land with the matching schema section in `system-knowhow/plugins.md`.
