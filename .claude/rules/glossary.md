...
- **A name must describe what the thing is NOW.** The rules above keep one name consistent across layers; this one keeps that name *true* as the thing grows. A file, module, type, function, or field whose scope has outgrown its name is a name that actively misleads, and it misleads hardest for an LLM, which routes on the name and the description rather than by reading the whole file. When you widen something's scope, ask whether its name still covers it, and if it doesn't, **rename it in the same change that widened it** rather than filing it. Per the cross-layer bullet above, the rename touches every surface together, and a half-rename is worse than the original drift. This is not a licence for a retroactive sweep: like `.claude/rules/no-em-dashes.md`, it binds names as you touch them. Worked example, 2026-08-02: `system-knowhow/building-a-trigger.md` had grown to cover editing, pausing, grouping, the `trigger.toml` read-model and running a trigger off-schedule, so "building" named one section of thirteen. It became `triggers.md`, and `building-a-plugin.md` (which already documented install / update / uninstall) became `plugins.md`. For an engine-shipped knowhow file the name is also the `load_knowhow` id, so the rename means sweeping every reference, including the hardcoded routes in the engine system prompt. Distinct from `CLAUDE.md`'s "never use generic names like `app.md`", which bans a name that was never specific; this bans one that stopped being accurate.
