...
A CC session must NOT run `npm install` / `npm ci` in its worktree because `node_modules` looks missing. The engine provisioned it before the session started, so a reinstall is pure waste.

Provisioning lives in `crates/lucidos-engine/src/engine/agent_session/run_session/spawn_context.rs`, over `node_modules_setup::{has_install_marker, member_node_modules_links}`. For every **Lucidos-source** thread (NOT external-repo, NOT app-coding-agent) the engine **hardlinks** main's installed tree into the worktree on spawn. That is `cp -al`, one to two seconds and zero disk, with a `cp -a` fallback across filesystems. It falls back to a cold `npm ci --prefer-offline` only when main itself has no install to copy.

The committed lockfile hoists **everything** to the worktree-ROOT `node_modules`, `vitest` included. So the root tree is the whole install, and `ls <worktree-root>/node_modules/.package-lock.json` is the one check that tells you provisioning worked.

Two traps follow from that:

- **Do not look for `vitest` under `crates/lucidos-app/node_modules`.** That directory holds only Vite's `.vite` / `.vite-temp` caches, so a check there calls a healthy tree broken and invites the forbidden reinstall. It was the real home of `vitest` until the 4.x hoist conflict cleared, which is why older plans still describe it that way.
- **A member tree never carries a `.package-lock.json` marker**, since npm writes that only at the install root. `has_install_marker` is therefore the root-tree check, and `member_node_modules_installed` gates the member link on a non-dot entry. The member loop is a safety net for a dep npm cannot hoist, and links nothing while the lockfile stays flat.
