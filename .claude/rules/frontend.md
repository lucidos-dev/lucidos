...
- **The delay gate has NO exception, and the transcript is the scar that proves it.** A timer never preempts running JavaScript, so a response landing inside `SPINNER_DELAY_MS` cancels a loader that never got a frame. A long synchronous render then paints nothing, and the transcript hit exactly that. The fix tried twice was a *hold*: raise the skeleton ahead of the gate, then apply the rows. It was reverted, and ADR 0081 records why in full. A hold's two steps ARE the two reported symptoms.
  - Raising the loader early is showing it instantly; blocking the main thread right after is having no transition. Attack the render cost instead, by windowing, chunking, or moving work off the main thread.

- **An element that animates must SURVIVE the conditional renders around it, and that means a `key`.** A remounted element cannot run a CSS transition. The browser has no previous value to interpolate from, so the fade becomes a snap. Preact matches unkeyed children by index. Two returns whose trees differ by a leading element therefore rebuild everything under that index. `ThreadView` is the worked example, and it cost three rounds of reports to find.
  - **Only a cold open crosses the two**, so it reproduced on the iOS PWA and almost never on desktop. Its cold-open return draws a bare wrap; the main return leads with a header. The PWA is evicted constantly, so nearly every open is cold. On desktop you click between threads already in `threadMap`. Key the shared nodes in BOTH returns, pinned by `components/chat/__tests__/thread-view-frame-identity.test.ts`.
  - **A loader the gate let through must be OPAQUE, never given an entrance fade.** Resting it transparent and ramping it up is a SECOND gate stacked on the delay gate, and the two compound. On a wait of a few hundred ms the loader never reaches a legible opacity. The user sees the bare pane through it and reports a blank screen with NO loader. One gate decides whether a loader is owed, and once it says yes, show it.
...
**Skeletons are self-skeletonizing: every `Loadable`-backed list/tree surface ships one, mirroring the real layout by construction.** Don't hand-draw generic shimmer bars or a separate skeleton tree, which drifts the moment the row changes. Build it from the row's OWN component, so the leaves render real content when loaded and `.sk-bar` shimmer inside the provider. The loaded path is then unchanged, and the two cannot fall out of sync.

- **Wire the row.** Give it optional props and a `useSkeleton()` check. Route its text through `<SkText>`, and its buttons, chips, icons and dots through `<SkBlock>` (both from `components/shared/Skeleton.tsx`). Gate handlers and optional structure on `!sk` / `(sk || cond)`.
...
## A UI Surface Owns Its Subscription

**All UI state is updated through SSE events.** The bullet above binds the emitting half. This is the receiving half, and it is enforced the same way: by a test, not by prose. Full reasoning in ADR 0118.

- **A surface that reads state on mount subscribes to the event that mutates it.** Reading a store signal IS the subscription, so a surface backed by one needs nothing more. A surface that fetches its own data (a settings page) subscribes through a **version counter** the dispatcher bumps: `useVersionedRefresh(xVersion.value, paused, reload)`. Precedents: `backupPreferencesVersion`, `mcpServersVersion`, `webhooksVersion`, `permissionGrantsVersion`.
- **Never a poll, an interval, a focus or visibility listener, or an invalidate-on-navigation.** Each makes one symptom go away and leaves the rule unenforced, so the next surface repeats the bug. `store/actions/sse-event-coverage.test.ts` scans the subscriber modules for all four.
- **A form field over entity state uses `useServerBackedField`** (`hooks/useServerBackedField.ts`). Untouched, it returns the served value and holds no copy, so a frame repaints it. Touched, it keeps the user's draft. **Seeding a `useState` from an entity is the bug this replaces**: the initializer runs once per mount, so a component keyed on an entity id keeps its first snapshot for good. That is what left the trigger detail page showing an old group after three `update_trigger` calls landed.
- **A whole-editor draft pauses the re-read instead** (`AllowlistEditor`, the app source editor). Pause, never drop: the frame lands when the draft is saved or reverted.
- **Every `SystemEvent` is answered or written down.** `sse-event-coverage.test.ts` reads `RESERVED_TYPE_NAMES` out of the Rust source. Each name needs an arm in `thread-sync.ts` / `entityReferences.ts`, or a row in that test's `NO_UI_STATE` table. A new variant costs one line; which line is the decision.
...
**Every header row centres its title on its PANE, and the reserve that keeps it off the flanking clusters is DERIVED.** All four are the same shape now (the desktop drawer's `.threads-header-title`, the Conversation pane's `.pane-header-brand-label`, the Canvas pane's `.pane-header-content-title`, and `.header-nav-cluster` on both mobile rows): the leading and trailing clusters stay in flow at the row's ends, and the title/brand cluster is absolutely centred over them at a `clamp(min-span, 100% - 2 * <reserve>, span)`. Two rules follow, and both are scars.

- **Centre on the pane, not on the gap between the clusters.** A `flex: 1` middle centres the title on whatever the two clusters leave, so it moves whenever either changes width. The Canvas row was the last one built that way and was reported for it (2026-08-13): its trailing cluster is 1 to 3 icon boxes wide depending on the content view, most visibly as the ⋯ overflow trigger appears and disappears, and the title and both chevrons slid with it.
- **Derive the reserve from the widest cluster the row can hold; never guess it.** It is written as icon boxes and gaps against `--header-icon-box`, and a source scan checks the copies (`styles/__tests__/content-header-title-centring.test.ts`, `header-band-centering.test.ts`). The clamp IS the non-overlap guarantee that `min-width: 0` gave the flex zone, so a guessed constant is not a cosmetic slip: the arrangement this replaced the FIRST time was a `translate(-50%)` title with `max-width: calc(100% - 12rem)`, and the guess is what let the title slide under the icons. Below the clamp's min-span arm the ends do reach the clusters, and there `useHeaderActionCollapse` folds the actions into ⋯ instead.
...
4. A `click` arriving **without** a preceding outside pointerdown (synthetic `HTMLElement.click()` from a keyboard shortcut, an e2e driver, or any programmatic source) falls into a click-capture fallback: same `isOutsidePointerTarget` check, same `onClose()` + swallow. Without this branch the contract silently breaks for any caller driving dismiss via synthetic clicks (`e2e/overlay-dismiss-swallow.spec.ts`, over the drawer row's overflow menu, is the canary; it used to be the thread filter, which is a pane panel now and no longer an overlay at all). **The one carve-out: a click dispatched WITHIN an inside click's dispatch is inside too**, for exactly that dispatch: capture opens the window on the inside click and a document-level BUBBLE listener closes it when that same event object comes back up, with a task-scoped backstop for a handler that stops propagation. A boolean on a timer instead would exempt every click in the rest of the task. An overlay whose action is `someInput.click()` on an element outside its panel would otherwise have its own action read as an outside click and `preventDefault()`ed, and when the target is a file input the cancelled default action IS the file chooser, so the item does nothing with nothing logged (the composer attach menu's File item, dead from 2026-05-19 to 2026-08-13). Deferring the `.click()` out of the gesture is not the alternative fix: that drops transient user activation, which is what makes a file picker unreliable on iOS.
