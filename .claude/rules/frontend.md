...
- Use `ApiError` + `toFailed()`. Use `useDelayedLoading(loadable)` for spinners (`SPINNER_DELAY_MS` = 300ms delay)
- **Never render a bare `<div class="loading-spinner" />` immediately.** When the spinner lives inside a kept container (modal frame, empty state), use `<DelayedSpinner />` from `components/shared/DelayedSpinner.tsx` — it mounts the spinner only after `SPINNER_DELAY_MS`, so fast loads never flash it. For non-`Loadable` boolean waits (or longer fuses like an 8s "tap to reload"), `useDelayedFlag(active, delayMs)` from `hooks/useDelayedLoading.ts` is the underlying hook.
...
The desktop focus half is **signal-only**, matching the pointer-down `focusPane`: it sets `focusedPane` but does not move DOM focus, so the first Tab pulls focus in (`paneTabTarget`'s `activeIndex < 0` branch) without yanking focus mid-navigation. Its mirror lives in `focusThread` (`store/actions/threads.ts`): navigating to a thread re-activates the **Threads** pane group (`focusedPane = 'thread'`) — but only from the cross-group case (`focusedPane === 'content'`), so an existing `'drawer'`/`'thread'` focus is left alone and drawer ↑/↓ browsing isn't disturbed.
