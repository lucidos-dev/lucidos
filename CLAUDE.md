...
- **Local specifics live in [`WORKSPACES.md`](WORKSPACES.md).** A machine can run several gateways, each with its own workspaces and Postgres. Machine- and team-specific details live there: which named workspaces hold live data, local browser and test setup. It is internal-only, so put machine paths and personal setup there and never in this file. It also holds the **private-data denylist**, the exact tokens the release guard scans for. That block is the single sanctioned place for them (`.claude/rules/no-private-data.md`).

## Coding-Agent Operational Rules

- **Don't reinstall `node_modules`: the engine already provisioned it.** On spawn it hardlinks the main repo's installed trees into the worktree, for every Lucidos-source thread (`agent_session/run_session/spawn_context.rs`). The lockfile hoists everything to the **worktree-root** tree, so `npx tsc --noEmit` and `npm test` both work. Never run `npm install`/`npm ci` yourself: it is redundant, and a bare `npm install` rewrites the committed lockfile (ADR 0020). External-repo and app-coding-agent threads are the only spawns the engine skips. One trap when checking the tree is there:
  - **`vitest` lives at the ROOT, in `node_modules/vitest`.** Judge the tree by `ls <root>/node_modules/.package-lock.json`. `crates/lucidos-app/node_modules` holds only Vite's cache, so looking for vitest there calls a healthy tree broken.
...
- **Never `pgrep` to wait.** Coding-agent sessions share the host process namespace, so `pgrep -f "cargo test ..."` matches *every* session's tests, and such a poll wedges until the Bash timeout fires. There is no blocking wait tool, and a job you background dies with your turn. Run anything under 10 minutes in the foreground with `timeout: 600000`; for longer, `lucidos background-task run -- '<cmd>'` and end your turn. To wait in shell, capture the PID directly: `cargo test ... & BG=$!; wait $BG`.
- **Never run two `cargo test` invocations against the same worktree in parallel.** `lucidos-engine` is heavy, so two overlapping runs fork two rustc trees on one crate and OOM-kill the host. Run one at a time: in the foreground, or as one background task. Never both. Narrow the filter to a *specific test module* (`engine::agent_session::lifecycle::tests::`) rather than a broad prefix, which cuts runtime and blast radius even for a single run.
...
- **Prefer `Glob`/`Grep` over `ls`/`grep` with absolute paths.** A remembered path goes stale the moment a refactor moves it. The shell form then fails with "no such file or directory" where `Grep pattern: 'pat'` finds the new location. Reach for them for any path you did not just see this turn.
...
- **Fix root causes, not symptoms.** Trace a bug to its originating layer; never paper over it with a downstream filter. Prefer the thorough, long-term fix over a narrow patch, and recommend it first.
