---
name: CognOS CLI (`cognos`)
description: Shell command available on PATH for any subprocess CognOS spawns (Python, bash, Claude Code) — writes files under `data/`, emits and queries domain events. Prefer this over hand-rolling HTTP calls back to the engine.
---

# `cognos` CLI

A shell command (`cognos`) available on the `PATH` of every subprocess CognOS spawns — Python scripts, bash scripts, Claude Code sessions. Use it whenever a script needs to:

- write files into the workspace's `data/` directory
- emit or query domain events on the workspace's event store

The CLI is a thin Rust wrapper around the engine's HTTP API and filesystem conventions — for app UI usage see the JS [`cognos.data.*`](./js-sdk.md) reference. Scripts should always prefer the CLI over hand-rolling HTTP calls back to the engine.

## When to use this

- **Python scripts** (`apps/<name>/scripts/*.py`, `triggers/<name>/scripts/*.py`, `knowhow/*/scripts/*.py`) — invoke via `subprocess.run(['cognos', 'data', 'write', ...])`. The CLI is on PATH and `COGNOS_WORKSPACE` is set automatically.
- **Bash scripts** (same locations, `*.sh`) — call `cognos` directly. PATH is set up before the script runs.
- **CC subprocesses** — running in a worktree under `<workspace>/.cognos/worktrees/<id>/`, where `Write`/`Edit` lands in the worktree (not the workspace), so dev-server links 404. Use `cognos data write` instead.

For app UIs (in the browser) keep using the JS SDK (`cognos.data.*`, `cognos.events.*`). The CLI is for shell / subprocess contexts only.
...
```bash
$ cognos data path artifacts/ua-analysis/foo/report.html
/Users/.../workspaces/work/data/artifacts/ua-analysis/foo/report.html

$ cognos data path report.html
/Users/.../workspaces/work/data/artifacts/report.html

$ cognos data path knowhow/myapp/notes.md --mkdir
...
### `cognos data write <relative> [--from <local-path> | -]`
...
```bash
$ cognos events query --type AnalysisCompleted --limit 1 | jq '.[0]'
{
  "id": "...",
...
1. **Walk up from `$PWD`** looking for the first ancestor directory that contains a `.cognos/ports` file. That ancestor is the workspace.
2. **Fall back to `$COGNOS_WORKSPACE`** environment variable. The engine sets this on every spawned subprocess (Python, bash, CC), so the fallback always works in the engine-spawned case.

The walk-up step naturally finds the right workspace from inside a worktree:

```
<workspace>/.cognos/worktrees/<id>/foo/bar     <- $PWD
<workspace>/.cognos/worktrees/<id>/foo
<workspace>/.cognos/worktrees/<id>
<workspace>/.cognos/worktrees
<workspace>/.cognos                            <- no .cognos/ports here
<workspace>                                    <- .cognos/ports found, stop
```
...
Both calls speak the same `data/`-rooted path convention, so an SSE listener that does `cognos.data.url(payload.artifact)` in the frontend will resolve the link correctly.

### Idempotent skip-if-already-done

```bash
LATEST=$(cognos events query --type DataImported --limit 1 | jq -r '.[0].payload.date // empty')
...
cognos events emit DataImported --summary "Imported $(date +%Y-%m-%d)" \
...
1. Symlinks the bundled `cognos` binary into `<workspace>/.cognos/bin/cognos` (idempotent — safe to call on every spawn).
2. Prepends `<workspace>/.cognos/bin` to `PATH`.
3. Sets `COGNOS_WORKSPACE=<workspace>` so the CLI's fallback always resolves.

For CC sessions it additionally drops a skill file at `<worktree>/.claude/skills/cognos-cli/SKILL.md` so CC discovers the CLI via its normal skill mechanism. A one-line reminder ("Use the `cognos` CLI for any data-dir writes or event emits.") in your trigger/app prompt is still good belt-and-braces.

## Implementation

- Source: `crates/cognos-cli/`
- Shared engine wiring: `crates/cognos-engine/src/runtime/cognos_cli.rs` — `cognos_cli_dir` discovers the binary, `ensure_workspace_bin_symlink` installs the workspace-relative symlink, `workspace_script_env_vars` builds the env var bundle.
